 ...
 */
function psmt_activity_get_attached_media_ids( $activity_id ) {
	return apply_filters( 'psmt_activity_get_attached_media_ids', bp_activity_get_meta( $activity_id, '_psmt_attached_media_id', false ), $activity_id );
}
 ...
 * @return array
 */
function psmt_activity_update_attached_media_ids( $activity_id, $media_ids ) {

	foreach ( $media_ids as $media_id ) {
		bp_activity_add_meta( $activity_id, '_psmt_attached_media_id', $media_id );
	}
 ...
 * @return boolean
 */
function psmt_activity_delete_attached_media_ids( $activity_id ) {
	return bp_activity_delete_meta( $activity_id, '_psmt_attached_media_id' );
}
 ...
 * @return array of media ids.
 */
function psmt_activity_get_displayable_media_ids( $activity_id ) {

	$media_ids = psmt_activity_get_attached_media_ids( $activity_id );
	$max       = psmt_activity_get_media_display_cap();
 ...
 * @return mixed
 */
function psmt_activity_get_media_display_cap() {
	return psmt_get_option( 'activity_media_display_limit', 6 );
}
 ...
 */
function psmt_activity_has_media( $activity_id = null ) {

	if ( ! $activity_id ) {
		$activity_id = bp_get_activity_id();
	}

	return psmt_activity_get_attached_media_ids( $activity_id );
}

/**
 * Returns the id of the gallery associated with this activity
 * _psmt_gallery_id meta key is added for activity uploads as well as single gallery activity/comment
 *
 * If it is a single gallery activity(comments on single gallery page), there won't exist the meta _psmt_media_id
 *
 ...
 * The only way to differentiate these two types of activity is to check for the presence of the _psmt_attached_media_ids meta
 *
 *  If a new activity is created by posting on single media page(comments), It does not have _psmt_gallery_id associated with it
 *
 * @param int $activity_id related activity id.
 *
 * @return int gallery id
 */
function psmt_activity_get_gallery_id( $activity_id ) {
	return bp_activity_get_meta( $activity_id, '_psmt_gallery_id', true );
}
 ...
 */
function psmt_activity_update_gallery_id( $activity_id, $gallery_id ) {
	return bp_activity_update_meta( $activity_id, '_psmt_gallery_id', $gallery_id );
}
 ...
 * @return  boolean
 */
function psmt_activity_delete_gallery_id( $activity_id ) {
	return bp_activity_delete_meta( $activity_id, '_psmt_gallery_id' );
}
 ...
 * for activity uploaded media please see _psmt_attached_media_id
 *
 ...
 * see _psmt_attached_media_id for the same
 *
 ...
 * @return mixed
 */
function psmt_activity_get_media_id( $activity_id ) {
	return bp_activity_get_meta( $activity_id, '_psmt_media_id', true );
}
 ...
 * @return bool|int
 */
function psmt_activity_update_media_id( $activity_id, $media_id ) {
	return bp_activity_update_meta( $activity_id, '_psmt_media_id', $media_id );
}
 ...
 * @return bool
 */
function psmt_activity_delete_media_id( $activity_id ) {
	return bp_activity_delete_meta( $activity_id, '_psmt_media_id' );
}
 ...
 */
function psmt_activity_get_context( $activity_id ) {
	return bp_activity_get_meta( $activity_id, '_psmt_context', true );
}
 ...
 * @return bool|int
 */
function psmt_activity_update_context( $activity_id, $context = 'gallery' ) {
	return bp_activity_update_meta( $activity_id, '_psmt_context', $context );
}
 ...
 * @return mixed
 */
function psmt_activity_delete_context( $activity_id ) {
	return bp_activity_delete_meta( $activity_id, '_psmt_context' );
}

/**
 * Storing/retrieving psmt activity type(create_gallery|update_gallery|media_upload) etc in activity meta
 *
 * @param int $activity_id related activity id.
 *
 * @return mixed
 */
function psmt_activity_get_activity_type( $activity_id ) {
	return bp_activity_get_meta( $activity_id, '_psmt_activity_type', true );
}

/**
 * Update PsourceMediathek activity type
 *
 * @param int    $activity_id related activity id.
 ...
 * @return mixed
 */
function psmt_activity_update_activity_type( $activity_id, $type ) {
	return bp_activity_update_meta( $activity_id, '_psmt_activity_type', $type );
}

/**
 * Delete PsourceMediathek activity type
 *
 * @param  int $activity_id related activity id.
 *
 * @return mixed
 */
function psmt_activity_delete_activity_type( $activity_id ) {
	return bp_activity_delete_meta( $activity_id, '_psmt_activity_type' );
}
 ...
 */
function psmt_activity_mark_attached_media( $activity_id ) {

	if ( empty( $_POST['psmt-attached-media'] ) || ! is_user_logged_in() ) {
		return;
	}

	// let us process.
	$media_ids = $_POST['psmt-attached-media'];
	...
	foreach ( $media_ids as $media_id ) {
		...
		psmt_delete_media_meta( $media_id, '_psmt_is_orphan' ); // or should we delete the key?
	}

	psmt_activity_update_attached_media_ids( $activity_id, $media_ids );

	psmt_activity_update_context( $activity_id, 'gallery' );
	psmt_activity_update_activity_type( $activity_id, 'media_upload' );
	// Keep it disabled by default.
	if ( count( $media_ids ) == 1 && apply_filters( 'psmt_use_single_media_upload_as_media_activity', false ) ) {
		psmt_activity_update_media_id( $activity_id, $media_id );
	}
	...
	if ( 'groups' === $activity->component && psmt_is_active_component( 'groups' ) && 'groups' !== $media->component ) {

		$group_wall_gallery = psmt_get_context_gallery( array(
			'component'    => 'groups',
		...
		if ( $group_wall_gallery ) {
			...
			// loop and move.
			foreach ( $media_ids as $media_id ) {
				psmt_move_media( $media_id, $group_wall_gallery->id );
				// clear media cache(details have changed).
				psmt_clean_media_cache( $media_id );
			}
		}
		// refetch media.
		$media = psmt_get_media( $media->id );
	}
	...
	// also update this activity and set its action to be psmt_media_upload
	// $activity->component = buddypress()->psourcemediathek->id;
	$activity->type = 'psmt_media_upload';
	$activity->save();

	// save activity privacy.
	$status_object = psmt_get_status_object( $media->status );
	...
	do_action( 'psmt_activity_media_marked_attached', $media_ids );
}
 ...
 * @return boolean
 */
function psmt_record_activity( $args = null ) {
	...
	if ( ( ! $args['gallery_id'] && ! $args['media_id'] )
	     || ! psmt_is_enabled( $args['component'], $args['component_id'] )
	     || ! $args['component_id']
	) {
		return false;
	}
	...
	if ( $args['status'] ) {
		$status_object = psmt_get_status_object( $args['status'] );
	...
	$activity_args = apply_filters( 'psmt_record_activity_args', $activity_args, $default );
	...
	if ( $type ) {
		psmt_activity_update_activity_type( $activity_id, $type );
	}
	...
	if ( $gallery_id ) {
		psmt_activity_update_gallery_id( $activity_id, $gallery_id );
	}
 ...
 */
function psmt_activity_comments( $args = '' ) {
	echo psmt_activity_get_comments( $args );
}
 ...
 * @return bool|mixed
 */
function psmt_activity_get_comments( $args = '' ) {
	...
	psmt_activity_recurse_comments( $activities_template->activity );
}
 ...
 * @return bool|mixed
 */
function psmt_activity_recurse_comments( $comment ) {
	global $activities_template;
	...
	$template = psmt_locate_template( array( 'buddypress/activity/comment.php' ), false, false );
 ...
 * @return mixed
 */
function psmt_activity_get_associated_comment_id( $activity_id ) {
	return bp_activity_get_meta( $activity_id, '_psmt_comment_id', true );
}
 ...
 * @return bool|int
 */
function psmt_activity_update_associated_comment_id( $activity_id, $value ) {
	return bp_activity_update_meta( $activity_id, '_psmt_comment_id', $value );
}
 ...
 * @return bool
 */
function psmt_activity_delete_associated_comment_id( $activity_id ) {
	return bp_activity_delete_meta( $activity_id, '_psmt_comment_id' );
}
 ...
 * @param int $activity_id activity id.
 */
function psmt_activity_create_comment_for_activity( $activity_id ) {

	if ( ! $activity_id || ! psmt_get_option( 'activity_comment_sync' ) ) {
		return;
	}
	...
	// this is not PsourceMediathek activity.
	...
	$comment_data = array(
		...
		'comment_type'    => psmt_get_comment_type(),
	);

	$comment_id = psmt_add_comment( $comment_data );

	// update comment meta.
	if ( $comment_id ) {

		psmt_update_comment_meta( $comment_id, '_psmt_activity_id', $activity_id );

		psmt_activity_update_associated_comment_id( $activity_id, $comment_id );
		...
		if ( $gallery_id && ! empty( $media_ids ) ) {
			...
			if ( function_exists( 'psmt_comment_update_attached_media_ids' ) ) {
				psmt_comment_update_attached_media_ids( $comment_id, $media_ids );
			}
		}
 ...
 * @return bool
 */
function psmt_delete_activity_meta_by_key_value( $key, $object_id ) {
 ...
 * @return bool
 */
function psmt_delete_activity_for_single_published_media( $media_id ) {
	...
	$query = $wpdb->prepare( $query, '_psmt_attached_media_id', $media_id, '_psmt_media_id', $media_id );
	...
	// delete  comments.
	$activity_comment_ids = psmt_delete_activity_comments( $to_delete_ids );
 ...
 */
function psmt_delete_activity_by_meta_key_value( $key, $object_id = null ) {
	...
	$activity_comment_ids = psmt_delete_activity_comments( $activity_ids );
 ...
 * @return array|bool
 */
function psmt_delete_activity_comments( $activity_ids ) {
	global $wpdb;
 ...
 * @return bool|int
 */
function psmt_activity_post_update( $args ) {
 ...
 */
function psmt_activity_post_group_update( $args = array() ) {
	if ( ! bp_is_active( 'activity' ) ) {
		return false;
	}
	...
	$activity_action  = sprintf( __( '%1$s hat ein Update in der Gruppe %2$s veröffentlicht', 'psourcemediathek' ), bp_core_get_userlink( $user_id ), '<a href="' . esc_url( bp_get_group_permalink( $bp->groups->current_group ) ) . '">' . esc_attr( $bp->groups->current_group->name ) . '</a>' );
	$activity_content = $r['content'];
