 ...
 */
class PSMT_Activity_Media_Cache_Helper {
	...
	public function cache( $activities ) {
		...
		foreach ( $activities as $activity ) {
			...
			// check for media ids.
			$attached_media_ids = psmt_activity_get_attached_media_ids( $activity->id );

			if ( ! empty( $attached_media_ids ) ) {
				$media_ids = array_merge( $media_ids, $attached_media_ids );
			}

			$associated_media_id = psmt_activity_get_media_id( $activity->id );
...
new PSMT_Activity_Media_Cache_Helper();
