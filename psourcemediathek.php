 ...
 * Description: PsourceMediathek ist das leistungsstärkste Medien-Plugin für BuddyPress. Es ermöglicht das Hochladen von Bildern (Fotos), Videos, Audios und Dokumenten und kann verwendet werden, um jede Art von Inhalt
 * hinzuzufügen. Es hat eine gut definierte API, um das Plugin zu erweitern.
 * License: GPL2 or above
 * Domain Path: /languages
 * Text Domain: psourcemediathek
 *
 * @package PsourceMediathek
 */

/*
Copyright 2015-2021 WMS N@W (https://n3rds.work)
Author – DerN3rd 
...
/**
 * The main PsourceMediathek Singleton class
 * you can access the singleton instance using psourcemediathek() function
 *
 * @see psourcemediathek()
 *
 * Life begins here
 */
class PsourceMediathek {


	/**
	 * Private Singleton instance of the PsourceMediathek class
	 *
	 * @var PsourceMediathek
	 */
	 ...
	 *
	 * @see PsourceMediathek::get_data( $key )
	 * @see PsourceMediathek::set_data( $key, $val )
	 *
	...
	/**
	 * File system absolute path to the psourcemediathek plugin eg. /home/xyz/public_html/wp-content/plugins/psourcemediathek/
	 *
	 * @see PsourceMediathek::get_path()
	 *
	 * @var string
	 */
	private $plugin_path;
	...
	/**
	 * Absolute url to the psourcemediathek plugin directory e.g http://example.com/wp-content/plugins/psourcemediathek/
	 *
	 * @var string
	 */
	private $plugin_url;
	 ...
	 *
	 * @var PSMT_Status[] array of status objects
	 */
	 ...
	 * Currently, It is same as Psourcemediathek::$statuses
	 *
	 * @var PSMT_Status[] array of status objects which are valid for gallery
	 */
	 ...
	 *
	 * @var PSMT_Status[] array of status objects which are valid for Media
	 */
	 ...
	 *
	 * @var PSMT_Component[] array of Component objects where keys are component identifier
	 */
	 ...
	 *
	 * @var PSMT_Type[] array of Media|Gallery type object
	 */
	public $types = array();
	 ...
	 * It can be controlled via PsourceMediathek settings page.
	 *
	 * @var PSMT_Status[] array of status objects
	 */
	 ...
	 *
	 * @see psmt_register_storage_manager()
	 *
	 * @var PSMT_Storage_Manager[]
	 */
	...
	//
	/**
	 * Is it the gallery home, true when on component/psourcemediathek pages
	 *
	 * @var bool
	 */
	 ...
	 *
	 * @var bool whether PsourceMediathek theme compat moe is enabled
	 */
	private $using_theme_compat = false;


	/**
	 * PsourceMediathek constructor. Internally called to create the singleton instance.
	 */
	 ...
	 *
	 * @see psourcemediathek() for accessing the instance
	 * @return PsourceMediathek
	 */
	...
	public function setup() {
		...
		global $wpdb;
		// PsourceMediathek Logs table name.
		$this->store_table_name( 'logs', $wpdb->prefix . 'psmt_logs' );
		...
		// Load the PsourceMediathek core.
		add_action( 'plugins_loaded', array( $this, 'load' ), 0 );
		// Load translation files.
		add_action( 'init', array( $this, 'load_textdomain' ), 0 );
		add_action( 'update_option_psmt-settings', array( $this, 'flush_rewrite_rules_on_settings_update' ), 10, 2 );
	}


	/**
	 * Loads the PsourceMediathek Core Loader class
	 *
	 * Loading is handled by the PSMT_Core_Loader
	 */
	public function load() {

		require_once $this->plugin_path . 'psmt-loader.php';

		$loader = new PSMT_Core_Loader();
		$loader->load();

		do_action( 'psmt_loaded' );
	}
	...
	public function load_logger() {
		...
		require_once $path . 'core/logger/class-psmt-logger.php';
		require_once $path . 'core/logger/class-psmt-db-logger.php';
		require_once $path . 'core/logger/psmt-logger-functions.php';
	...
	public function do_activation() {
		...
		require_once $this->plugin_path . 'admin/psmt-admin-install.php';

		// psmt_upgrade_legacy_1_0_b1_activity();
		// post type functionality.
		require_once $this->plugin_path . 'core/common/psmt-common-functions.php';
		require_once $this->plugin_path . 'core/psmt-post-type.php';

		// store default settings if not already exists.
		add_option( 'psmt-settings', psmt_get_all_options() );
		// initialize post type( because we want to flush the rewrite rules).
		PSMT_Post_Type_Helper::get_instance()->init();
		...
		// schedule cron.
		require_once $this->plugin_path . 'core/common/psmt-cron.php';
		// schedule.
		psmt_schedule_cron_job();
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'psourcemediathek', false, dirname( $this->basename ) . '/languages' );
	}


	/**
	 * Get the url of the PsourceMediathek plugin directory ( e.g http://site.com/wp-content/plugins/psourcemediathek/)
	 *
	 * @return string
	 */
	...
	/**
	 * Get the absolute path to the psourcemediathek plugin directory( e.g /home/xyz/public_html/wp-content/plugins/psourcemediathek/)
	 *
	 * @return string
	 */
	...
	/**
	 * Get the relative path of this file from the plugins directory e.g psourcemediathek/psourcemediathek.php
	 *
	 ...
	 *
	 * @param string $type psmt_get_gallery_post_type() or psmt_get_media_post_type().
	 */
	...
	/**
	 * Get the given PSMT_Menu object by the menu name
	 *
	 ...
	 * @param PSMT_Menu $menu The menu to add.
	 */
	...
	/**
	 * Is PsourceMediathek integration enabled for BuddyPress
	 *
	...
	public function is_bp_enabled() {
		...
		if ( $this->is_bp_active() && ( psmt_is_active_component( 'groups' ) || psmt_is_active_component( 'members' ) ) ) {
			return true;
		}
...
/**
 * A shortcut function to allow access to the singleton instance of the PsourceMediathek
 *
 * @return PsourceMediathek
 */
function psourcemediathek() {
	return PsourceMediathek::get_instance();
}

// Initialize PsourceMediathek.
psourcemediathek();
