...
$ext = psmt_get_file_extension( $src );
// valid extension.
if ( $ext && psmt_is_doc_viewer_enabled( $media ) && psmt_doc_viewer_supports_file_type( 'gdoc', $ext ) ) {
	// $ext = strtolower( $ext );
	...
	$html = sprintf( '<a href="%s">%s</a>', esc_attr( $src ), esc_attr( basename( psmt_get_media_path('', $media ) ) ) );
}
