 ...
 * @type @exp;window@pro;psmt
 */
/* global wp, pluploadL10n, plupload, _wpPluploadSettings, psmt, _psmtData, _psmtUploadSettings, Backbone, jQuery, console, _ */

window.psmt = window.psmt || {};

(function (exports, $) {
    var Uploader;

    if (typeof _psmtUploadSettings === 'undefined') {
        return;
    }
    ...
    Uploader = function (options) {
        ...
        for (key in elements) {
            ...
            if (!this[key].prop('id')) {
                this[key].prop('id', '__psmt-uploader-id-' + Uploader.uuid++);//If there is no id, generate one for the element
            }
        ...
        error = function (message, data, file) {
            console.log("PSMT Error: " + message);
            if (file.attachment) {
            ...
            $(document).trigger( 'psmt:uploader:error', [self, message, data, file] );
        };
        ...
        (function (dropzone, supported) {
            ...
            if (!supported) {
                return dropzone.unbind('.psmt-uploader');
            }

            // 'dragenter' doesn't fire correctly,
            // simulate it with a limited 'dragover'
            dropzone.bind('dragover.psmt-uploader', function () {
                if (timer) {
                    clearTimeout(timer);
                }
            ...
            dropzone.bind('dragleave.psmt-uploader, drop.psmt-uploader', function () {
        ...
        this.uploader.bind('FilesAdded', function (up, files) {
            _.each(files, function (file) {
                ...
                $(document).trigger( 'psmt:uploader:file:added', [self, file] );
            });
            // trigger files enqueued.
            self.allFilesAdded(up);
            $(document).trigger( 'psmt:uploader:files:added', [self, up] );
            up.refresh();
            up.start();
        });
        ...
        this.uploader.bind('UploadProgress', function (up, file) {
            ...
            $(document).trigger( 'psmt:uploader:upload:progress', [self, file.attachment] );
        });
        ...
        this.uploader.bind('FileUploaded', function (up, file, response) {
            ...
            $(document).trigger( 'psmt:uploader:upload:success', [self, file.attachment] );
        });
        ...
        this.uploader.bind('UploadComplete', function (up, files) {
            ...
            $(document).trigger( 'psmt:uploader:upload:complete', [self, up, files] );
        });
        ...
        this.uploader.bind('BeforeUpload', function (up, file) {
            ...
            $(document).trigger( 'psmt:uploader:before:upload', [self, up, file] );
        });
    ...
    $.extend(Uploader, _psmtUploadSettings);
    ...
    $.extend(Uploader.prototype, {
        ...
        error: function (reason, data, file) {
            ...
            if (data && data.code == '-601' && psmt.notify != undefined && _psmtData.current_type) {
        ...
        added: function (file) {
            ...
            //$( '.psmt-uploading', this.feedback  ).show();//addClass( 'uploading' );
        },
        ...
        init: function () {
            ...
            this.clear_media_list();
            //jq('ul', this.media_list).append( jq( '#psmt-loader-wrapper'));
        ...
        refresh: function () {
            ...
            if (this.browser) {
                ...
                if (!attached) {
                    id = 'psmt-uploader-browser-' + this.uploader.id;

                    container = $('#' + id);
                    if (!container.length) {
                        container = $('<div class="psmt-uploader-browser" />').css({
                            position: 'fixed',
                            top: '-1000px',
                            left: '-1000px',
                            height: 0,
                            width: 0
                        }).attr('id', 'psmt-uploader-browser-' + this.uploader.id).appendTo('body');
                    }
...
function psmt_setup_uploader_file_types(psmt_uploader, type) {

    if (!_psmtData || !_psmtData.types) {
        return;
    }

    if (type === undefined && _psmtData.current_type !== undefined) {
        type = _psmtData.current_type;
    }
    ...
    //console.log(psmt_uploader);
    // for iOS, the getOption('filters') throws error.
    try {
        settings = psmt_uploader.uploader.getOption('filters');
    } catch (e) {
        settings = {};
        console.log(e);
    }

    settings.mime_types = [_psmtData.types[type]];

    psmt_uploader.uploader.setOption('filters', settings);

    if (psmt_uploader.dropzone) {
        jQuery(psmt_uploader.dropzone).find('.psmt-uploader-allowed-file-type-info').html(_psmtData.allowed_type_messages[type]);
        jQuery(psmt_uploader.dropzone).find('.psmt-uploader-allowed-max-file-size-info').html(_psmtData.max_allowed_file_size);
    }
}
 ...
 */
function psmt_get_attached_media() {

    return jQuery('body').data('psmt-attached-media');
}
 ...
 * @returns {undefined}
 */
function psmt_add_attached_media(media_id) {

    var $body = jQuery('body');
    var attached_media = $body.data('psmt-attached-media');
    ...
    $body.data('psmt-attached-media', attached_media);
 ...
 * @returns {Boolean}
 */
function psmt_remove_attached_media(media_id) {
...
function psmt_reset_attached_media() {
    jQuery('body').data('psmt-attached-media', '');
}