<?php
/**
 * Local Storage Manager for PsourceMediathek
 *
 ...
 */
class PSMT_Local_Storage extends PSMT_Storage_Manager {

	/**
	 * Singleton instance
	 *
	 * @var PSMT_Local_Storage
	 */
	...
	public function get_src( $size = null, $id = null ) {
		...
		if ( $id && is_a( $id, 'PSMT_Media' ) ) {
			$id = psmt_get_media_id( $id );
		}
	...
	/**
	 * Get the absolute path to a file ( file system path like /home/xyz/public_html/wp-content/uploads/psourcemediathek/members/1/xyz)
	 *
	...
	public function upload( $file, $args ) {
		...
		if ( ! function_exists( 'psmt_handle_upload_error' ) ) {

			function psmt_handle_upload_error( $file, $message ) {
				return array( 'error' => $message );
			}
		}

		$upload_error_handler = 'psmt_handle_upload_error';

		$file = apply_filters( 'psmt_upload_prefilter', $file );
		...
		if ( $test_size && ! ( $file['size'] > 0 ) ) {
			...
			if ( is_multisite() ) {
				$error_msg = _x( 'File is empty. Please upload something more substantial.', 'upload error message', 'psourcemediathek' );
			} else {
				$error_msg = _x( 'File is empty. Please upload something more substantial. This error could also be caused by uploads being disabled in your php.ini or by post_max_size being defined as smaller than upload_max_filesize in php.ini.', 'upload error message', 'psourcemediathek' );
			}
		...
		if ( $test_upload && ! @ is_uploaded_file( $file['tmp_name'] ) ) {
			return call_user_func( $upload_error_handler, $file, _x( 'Specified file failed upload test.', 'upload error message', 'psourcemediathek' ) );
		}
		...
		if ( $test_type ) {
			...
			if ( ( empty( $type ) || empty( $ext ) ) && ! current_user_can( 'unfiltered_upload' ) ) {
				return call_user_func( $upload_error_handler, $file, _x( 'Sorry, this file type is not permitted for security reasons.', 'upload error message', 'psourcemediathek' ) );
			}
		...
		if ( false === $moved ) {
			...
			return $upload_error_handler( $file, sprintf( _x( 'The uploaded file could not be moved to %s.', 'upload error message', 'psourcemediathek' ), $error_path ) );
		}
		...
		return apply_filters( 'psmt_handle_upload', array(
			'file' => $new_file,
			'url'  => $url,
			'type' => $type,
		), 'upload' );
	...
	public function upload_bits( $name, $bits, $upload ) {
		...
		if ( empty( $name ) ) {
			return array( 'error' => _x( 'Empty filename', 'upload error message', 'psourcemediathek' ) );
		}

		$wp_filetype = wp_check_filetype( $name );

		if ( ! $wp_filetype['ext'] && ! current_user_can( 'unfiltered_upload' ) ) {
			return array( 'error' => _x( 'Invalid file type', 'upload error message', 'psourcemediathek' ) );
		}
		...
		$upload_bits_error = apply_filters( 'psmt_upload_bits', array(
			'name' => $name,
			//'bits' => $bits,
			'path' => $upload['path'],
		) );
		...
		if ( ! wp_mkdir_p( dirname( $new_file ) ) ) {
			...
			$message = sprintf( _x( 'Unable to create directory %s. Is its parent directory writable by the server?', 'upload error message', 'psourcemediathek' ), dirname( $new_file ) );
		...
		if ( ! $ifp ) {
			return array( 'error' => sprintf( _x( 'Could not write file %s', 'upload error message', 'psourcemediathek' ), $new_file ) );
		}
	...
	public function generate_metadata( $attachment_id, $file ) {
		...
		if ( preg_match( '!^image/!', $mime_type ) && file_is_displayable_image( $file ) ) {
			...
			$sizes = psmt_get_media_sizes();

			$sizes = apply_filters( 'psmt_intermediate_image_sizes', $sizes, $attachment_id );
		...
		if ( ! empty( $metadata['image']['data'] ) ) {
			...
			if ( false === $uploaded['error'] ) {
				...
				if ( psmt_get_option( 'set_post_thumbnail' ) ) {
					psmt_update_media_meta( $attachment_id, '_thumbnail_id', $sub_attachment_id );
				}
				// set the cover id.
				psmt_update_media_cover_id( $attachment_id, $sub_attachment_id );
			}
		}
		...
		return apply_filters( 'psmt_generate_metadata', $metadata, $attachment_id );
	}
	 ...
	 *
	 * @param int|PSMT_Media   $media_id the media to be moved.
	 * @param int|PSMT_Gallery $to_gallery_id Destination gallery id.
	 *
	...
	public function move_media( $media_id, $to_gallery_id ) {
		...
		$from_gallery = psmt_get_gallery( $media->gallery_id );
		$to_gallery   = psmt_get_gallery( $to_gallery_id );
		...
		$attached_file = psmt_get_media_meta( $media_id, '_wp_attached_file', true );
		...
		if ( $attached_file ) {
			...
			psmt_update_media_meta( $media_id, '_wp_attached_file', $rel_path );
	...
	public function import_file( $file, $gallery_id ) {
		...
		if ( false === copy( $file, $new_file ) ) {
			return new WP_Error( 'file_not_copied', sprintf( __( 'Unable to copy file from %1$s to %2$s. Please check directory permission.', 'psourcemediathek' ), $file, $new_file ) );
		}
	...
	public function import_url( $url, $gallery_id ) {
		...
		if ( is_wp_error( $tmp ) ) {
			return new WP_Error( 'unable_to_download', __( 'There was a problem downloading media.', 'psourcemediathek' ) );
		}
		...
		if ( is_wp_error( $tmp ) ) {
			...
			return new WP_Error( 'unable_to_download_2', __( 'There was a problem downloading media.', 'psourcemediathek' ) );
		}

		$file_array['size'] = filesize( $tmp );
		$gallery = psmt_get_gallery( $gallery_id );
		$files = array();
		$files['_psmt_file'] = $file_array;
	 ...
	 *
	 * @see psmt_get_used_space
	 *
	 * @access private do not call it directly, use psmt_get_used_space instead
	 *
	...
	public function get_used_space( $component, $component_id ) {
		...
		$key = "psmt_space_used_by_{$component}_{$component_id}"; // transient key.
	...
	public function can_handle() {
		...
		if ( $_FILES['_psmt_file']['size'] < wp_max_upload_size() ) {
			return true;
		}
	...
	public function get_upload_dir( $args ) {
		...
		// now reset upload/sub dir, we have hardcoded psourcemediathek for now, if you want it to be changed, please create a ticket.
		$uploads['subdir'] = "/psourcemediathek/{$component}/{$component_id}";

		// make folder like /psourcemediathek/{groups|members}/{user_id or group_id}.
		if ( $args['gallery_id'] ) {
	...
	public function setup_upload_errors( $component ) {
		...
		$allowed_size = psmt_get_allowed_space( $component );

		$this->upload_errors = array(
			UPLOAD_ERR_OK         => _x( 'Great! the file uploaded successfully!', 'upload error message', 'psourcemediathek' ),
			UPLOAD_ERR_INI_SIZE   => sprintf( _x( 'Your file size was bigger than the maximum allowed file size of: %s', 'upload error message', 'psourcemediathek' ), $allowed_size ),
			UPLOAD_ERR_FORM_SIZE  => sprintf( _x( 'Your file was bigger than the maximum allowed file size of: %s', 'upload error message', 'psourcemediathek' ), $allowed_size ),
			UPLOAD_ERR_PARTIAL    => _x( 'The uploaded file was only partially uploaded', 'upload error message', 'psourcemediathek' ),
			UPLOAD_ERR_NO_FILE    => _x( 'No file was uploaded', 'upload error message', 'psourcemediathek' ),
			UPLOAD_ERR_NO_TMP_DIR => _x( 'Missing a temporary folder.', 'upload error message', 'psourcemediathek' ),
		);
	}
 ...
 * @return PSMT_Local_Storage
 */
function psmt_local_storage() {
	return PSMT_Local_Storage::get_instance();
}
