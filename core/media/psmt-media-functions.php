 ...
 * @return bool
 */
function psmt_is_file_upload_enabled( $context = '' ) {
	return apply_filters( 'psmt_file_upload_enabled', psmt_get_option( 'enable_file_upload', 1 ), $context );
}

/**
 * Check if given post is a valid PsourceMediathek media.
 *
 * Checks for post type + _is_psmt_media meta
 *
 * @param int $media_id media id.
 *
 * @return boolean
 */
function psmt_is_valid_media( $media_id ) {

	if ( psmt_get_media_meta( $media_id, '_psmt_is_psmt_media', true ) && ( get_post_type( $media_id ) == psmt_get_media_post_type() ) ) {
		return true;
	}
 ...
 * @return array
 */
function psmt_get_all_media_ids( $args = null ) {
 ...
 *
 * @param array $args see psmt_get_object_count() for details.
 *
 * @see psmt_get_object_count()
 * @see psmt_get_user_media_count_by_component() for a better performance alternative.
 *
 * @return int
 */
function psmt_get_media_count( $args ) {

	$args['post_status'] = 'inherit';

	return psmt_get_object_count( $args, psmt_get_media_post_type() );
}
 ...
 * @return bool|object
 */
function psmt_media_exists( $media_slug, $component, $component_id ) {

	if ( ! $media_slug ) {
		return false;
	}

	return psmt_post_exists( array(
		'component'    => $component,
		'component_id' => $component_id,
		'slug'         => $media_slug,
		'post_status'  => 'inherit',
...
function psmt_add_media( $args ) {
	...
	// we update it.
	if ( ! $sort_order && psmt_is_gallery_sorted( $gallery_id ) ) {
		// current max sort order +1.
		$sort_order = (int) psmt_get_max_media_order( $gallery_id ) + 1;
	}
	...
	// set component.
	if ( $component ) {
		wp_set_object_terms( $id, psmt_underscore_it( $component ), psmt_get_component_taxname() );
	}

	// set _component_id meta key user_id/gallery_id/group id etc.
	if ( $component_id ) {
		psmt_update_media_meta( $id, '_psmt_component_id', $component_id );
	}
	// set upload context.
	if ( $context && 'activity' === $context ) {
		// only store context for activity uploaded media.
		psmt_update_media_meta( $id, '_psmt_context', $context );
	}

	// set media privacy.
	if ( $status ) {
		wp_set_object_terms( $id, psmt_underscore_it( $status ), psmt_get_status_taxname() );
	}
	// set media type internally as audio/video etc.
	if ( $type ) {
		wp_set_object_terms( $id, psmt_underscore_it( $type ), psmt_get_type_taxname() );
	}

	if ( $storage_method && 'local' !== $storage_method ) {
		// keep storage manager info if it is not default.
		psmt_update_media_meta( $id, '_psmt_storage_method', $storage_method );
	}

	// add all extras here.
	if ( $r['is_orphan'] ) {
		psmt_update_media_meta( $id, '_psmt_is_orphan', 1 );
	}

	if ( $r['is_remote'] ) {
		psmt_update_media_meta( $id, '_psmt_is_remote', 1 );
	}

	if ( $r['is_raw'] ) {
		psmt_update_media_meta( $id, '_psmt_is_raw', 1 );
	}

	if ( $r['is_oembed'] ) {
		psmt_update_media_meta( $id, '_psmt_is_oembed', 1 );
	}

	if ( $r['source'] ) {
		psmt_update_media_meta( $id, '_psmt_source', $r['source'] );
	}

	if ( $r['embed_html'] ) {
		psmt_update_media_meta( $id, '_psmt_oembed_content', $r['embed_html'] );
		psmt_update_media_meta( $id, '_psmt_oembed_time', time() );
	}

	// is_uploaded
	// is_remote
	// mark as psourcemediathek media.
	psmt_update_media_meta( $id, '_psmt_is_psmt_media', 1 );

	wp_update_attachment_metadata( $id, psmt_generate_media_metadata( $id, $r['src'] ) );

	do_action( 'psmt_media_added', $id, $gallery_id );
 ...
 * @return int|boolean
 */
function psmt_update_media( $args ) {
	...
	if ( ! $sort_order && ! $post_data['menu_order'] && psmt_is_gallery_sorted( $gallery_id ) ) {
		// current max sort order +1.
		$sort_order = (int) psmt_get_max_media_order( $gallery_id ) + 1;
	}
	...
	if ( $r['is_remote'] ) {
		...
		psmt_delete_media_meta( $id, '_psmt_is_remote' );
	}
	...
	if ( $r['is_raw'] ) {
		psmt_update_media_meta( $id, '_psmt_is_raw', 1 );
	} elseif ( isset( $custom['_psmt_is_raw'] ) ) {
		// only delete if it was already set.
		psmt_delete_media_meta( $id, '_psmt_is_raw' );
	}

	if ( $r['is_oembed'] ) {
		psmt_update_media_meta( $id, '_psmt_is_oembed', 1 );
	} elseif ( isset( $custom['_psmt_is_oembed'] ) ) {
		// only delete if it was already set.
		psmt_delete_media_meta( $id, '_psmt_is_oembed' );
	}

	if ( $r['source'] ) {
		psmt_update_media_meta( $id, '_psmt_source', $r['source'] );
	} elseif ( isset( $custom['_psmt_source'] ) ) {
		// only delete if it was already set.
		psmt_delete_media_meta( $id, '_psmt_source' );
	}

	if ( $r['embed_html'] ) {
		psmt_update_media_meta( $id, '_psmt_oembed_content', $r['embed_html'] );
		psmt_update_media_meta( $id, '_psmt_oembed_time', time() );
	} elseif ( isset( $custom['_psmt_oembed_content'] ) ) {
		// only delete if it was already set.
		psmt_delete_media_meta( $id, '_psmt_oembed_content' );
		psmt_delete_media_meta( $id, '_psmt_oembed_time' );
	}
	...
	if ( $r['is_orphan'] ) {
		...
		// only delete if it was already set.
		psmt_delete_media_meta( $id, '_psmt_is_orphan' );
	}

	do_action( 'psmt_media_updated', $id, $gallery_id );
 ...
 *        -> PSMT_Deletion_Actions_Mapper::map_before_delete_attachment()
 *        -> do_action ( 'psmt_before_media_delete', $gallery_id )
 *        -> cleanup gallery
 *        .........
 *        .........
 *
 ...
 *        -> do_action( 'psmt_media_deleted', $gallery_id )
 *
 * @param int $media_id media to be deleted.
 *
 * @return mixed
 */
function psmt_delete_media( $media_id ) {
 ...
 *
 * @param int|PSMT_Media   $media_id to be moved.
 ...
 * @return bool
 */
function psmt_move_media( $media_id, $gallery_id, $override = array() ) {

	$storage = psmt_get_storage_manager( $media_id );
	// first move files.
 ...
 * @return WP_Error|int
 */
function psmt_import_attachment( $attachment_id, $gallery_id, $override = array() ) {
	return PSMT_Media_Importer::import_attachment( $attachment_id, $gallery_id, $override );
}
 ...
 * @return WP_Error|int
 */
function psmt_import_file( $file, $gallery_id, $override = array() ) {
	return PSMT_Media_Importer::import_file( $file, $gallery_id, $override );
}
 ...
 * @return boolean|int
 */
function psmt_update_media_order( $media_id, $order_number ) {
 ...
 */
function psmt_get_max_media_order( $gallery_id ) {
 ...
 */
function psmt_get_media_type_from_extension( $ext ) {
 ...
 * @return string
 */
function psmt_get_file_extension( $file_name ) {
 ...
 * @return bool
 */
function psmt_is_doc_viewer_enabled( $media ) {
	$enabled = psmt_get_option( 'gdoc_viewer_enabled', 1 );
	// by default is is enabled.
	return apply_filters( 'psmt_doc_viewer_enabled', $enabled, $media );
}
/**
 ...
 * @return bool
 */
function psmt_doc_viewer_supports_file_type( $viewer = 'gdoc', $ext = '' ) {
	...
	$supported = apply_filters( 'psmt_doc_viewer_supported_types', $supported, $viewer, $ext );
 ...
 * @return bool
 */
function psmt_is_doc_viewable( $media ) {

	$viewable = false;
	$media    = psmt_get_media( $media );
	if ( $media && psmt_is_doc_viewer_enabled( $media ) && psmt_doc_viewer_supports_file_type( 'gdoc', psmt_get_file_extension( psmt_get_media_src( '', $media ) ) ) ) {
		$viewable = true;
	}

	return apply_filters( 'psmt_doc_viewable', $viewable, $media );
}
 ...
 * @return array|null
 */
function psmt_media_to_json( $attachment ) {
	...
	$media = psmt_get_media( $attachment->ID );
	...
	$response = array(
		...
		'alt'           => psmt_get_media_title( $media ),
		...
		'meta'          => false,
		// 'thumbnail'		=> psmt_get_media_src('thumbnail', $media ).
	);
	...
	if ( $meta && ( 'audio' === $type || 'video' === $type ) ) {
		...
		$id = psmt_get_media_cover_id( $attachment->ID );
		...
		if ( ! empty( $id ) ) {
			...
			$url               = psmt_get_media_cover_src( 'thumbnail', $media->id );
			$width             = 48;
			$height            = 64;
			$response['image'] = compact( 'url', 'width', 'height' );
			$response['thumb'] = compact( 'url', 'width', 'height' );
	...
	if ( ! empty( $response['sizes'] ) && empty( $response['sizes']['thumbnail'] ) ) {
		$thumb_dimension                = psmt_get_media_size( 'thumbnail' );
		$url                            = psmt_get_media_cover_src( 'thumbnail', $media->id );
	...
	return apply_filters( 'psmt_prepare_media_for_js', $response, $attachment, $meta );
}
 ...
 * @return array
 */
function psmt_generate_media_metadata( $media_id, $src ) {
 ...
 * @return bool
 */
function psmt_media_record_activity( $args ) {
	...
	if ( ! apply_filters( 'psmt_media_do_record_activity', true, $args ) ) {
		return false;
	}
	...
	if ( 'public' === $status ) {
		$status = psmt_get_gallery_status( $gallery );
	}
 ...
 * @return boolean
 */
function psmt_show_media_description( $media = null ) {

	$media = psmt_get_media( $media );

	$show = psmt_get_option( 'show_media_description' ); // under theme tab in admin panel.

	return apply_filters( 'psmt_show_media_description', $show, $media );
}
 ...
 */
function psmt_get_title_desc_from_meta( $type, $meta ) {
	$title = $content = '';
	// match mime type.
	if ( preg_match( '#^audio#', $type ) ) {
		...
		if ( ! empty( $title ) ) {
			...
			if ( ! empty( $meta['album'] ) && ! empty( $meta['artist'] ) ) {
				...
				$content .= sprintf( __( '"%1$s" from %2$s by %3$s.', 'psourcemediathek' ), $title, $meta['album'], $meta['artist'] );
			} elseif ( ! empty( $meta['album'] ) ) {
				/* translators: 1: audio track title, 2: album title */
				$content .= sprintf( __( '"%1$s" from %2$s.', 'psourcemediathek' ), $title, $meta['album'] );
			} elseif ( ! empty( $meta['artist'] ) ) {
				/* translators: 1: audio track title, 2: artist name */
				$content .= sprintf( __( '"%1$s" by %2$s.', 'psourcemediathek' ), $title, $meta['artist'] );
			} else {
			...
			if ( ! empty( $meta['artist'] ) ) {
				...
				$content .= sprintf( __( '%1$s by %2$s.', 'psourcemediathek' ), $meta['album'], $meta['artist'] );
			} else {
				$content .= $meta['album'] . '.';
			}
		} elseif ( ! empty( $meta['artist'] ) ) {
		...
		if ( ! empty( $meta['year'] ) ) {
			$content .= ' ' . sprintf( __( 'Released: %d.', 'psourcemediathek' ), $meta['year'] );
		}
		...
		if ( ! empty( $meta['track_number'] ) ) {
			...
			if ( isset( $track_number[1] ) ) {
				$content .= ' ' . sprintf( __( 'Track %1$s of %2$s.', 'psourcemediathek' ), number_format_i18n( $track_number[0] ), number_format_i18n( $track_number[1] ) );
			} else {
				$content .= ' ' . sprintf( __( 'Track %1$s.', 'psourcemediathek' ), number_format_i18n( $track_number[0] ) );
			}
		}

		if ( ! empty( $meta['genre'] ) ) {
			$content .= ' ' . sprintf( __( 'Genre: %s.', 'psourcemediathek' ), $meta['genre'] );
		}
