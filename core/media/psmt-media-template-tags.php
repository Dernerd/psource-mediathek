...
function psmt_load_media_view( $media = null ) {
	...
	if ( ! $view ) {
		printf( __( 'Es ist kein Ansichtsobjekt registriert, das die Anzeige des Inhalts vom Typ <strong>%s</strong> übernimmt', 'psourcemediathek' ), strtolower( psmt_get_type_singular_name( $media->type ) ) );
	} else {
		$view->display( $media );
	}
}
...
function psmt_get_adjacent_media_link( $format, $link, $media_id = null, $previous = false ) {
	...
	if ( empty( $title ) ) {
		$title = $previous ? __( 'Vorherige', 'psourcemediathek' ) : __( 'Nächstes', 'psourcemediathek' );
	}
...
function psmt_get_media_action_links( $media = null ) {
	...
	if ( psmt_user_can_delete_media( $media ) ) {
		$links['delete'] = sprintf( '<a href="%1$s" title="' . __( 'Lösche %2$s', 'psourcemediathek' ) . '" class="confirm psmt-confirm psmt-delete psmt-delete-media">%3$s</a>', psmt_get_media_delete_url( $media ), psmt_get_media_title( $media ), __( 'Löschen', 'psourcemediathek' ) );
	}
