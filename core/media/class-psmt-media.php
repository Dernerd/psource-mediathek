...
/**
 * PsourceMediathek Media class.
 *
 * Please do not use this class directly instead use psmt_get_media
 *
 * @see psmt_get_media
 *
 * @since 1.0.0
...
class PSMT_Media {
	 ...
	 * Constructor.
	 *
	 * @param int|PSMT_Media|array|null $media medi id, array or object.
	 */
	...
	public function map_object( $media ) {
		...
		if ( is_a( $media, 'PSMT_Media' ) ) {
			// should we map or should we throw exception and ask them to use the psmt_get_media.
			_doing_it_wrong( 'PSMT_Media::__construct', __( 'Please do not call the constructor directly, Instead the recommended way is to use psmt_get_media', 'psourcemediathek' ), '1.0' );
			return;
		}
	 ...
	 * Get field map
	 *
	 * Maps WordPress post table fields to PSMT_Media field
	 *
	 * @return array
	 */
	...
	public function __isset( $key ) {
		...
		if ( 'component' == $key ) {
			$this->set( $key, psmt_get_object_component( $this->id ) );
			$exists = true;
		} elseif ( 'type' == $key ) {
			$this->set( $key, psmt_get_object_type( $this->id ) );
			$exists = true;
		} elseif ( 'status' == $key ) {
			$this->set( $key, psmt_get_object_status( $this->id ) );
			$exists = true;
		}
		...
		// eg _psmt_is_remote etc on call of $obj->is_remote.
		return metadata_exists( 'post', $this->id, '_psmt_' . $key );
	}
	...
	public function __get( $key ) {
		...
		$value = psmt_get_media_meta( $this->id, '_psmt_' . $key, true );
 ...
 *
 * @return PSMT_Media|null PSMT_Media on success or null on failure
 */
function psmt_get_media( $media = null, $output = OBJECT ) {

	$_media        = null;
	$needs_caching = false;

	// if a media is not given but we are inside the media loop.
	if ( empty( $media ) && psourcemediathek()->current_media ) {
		$media = psourcemediathek()->current_media;
	}
	...
	if ( is_a( $media, 'PSMT_Media' ) ) {
		...
		$_media = psmt_get_media_from_cache( $media );

		if ( ! $_media ) {
			$_media        = new PSMT_Media( $media );
			$needs_caching = true;
		}
	} elseif ( is_object( $media ) ) {
		$_media = psmt_get_media_from_cache( $media->ID );
	...
	if ( $needs_caching && ! empty( $_media ) && $_media->id ) {
		psmt_add_media_to_cache( $_media );
	}
 ...
 * @return PSMT_Media
 */
function psmt_get_media_from_cache( $media_id ) {
	return wp_cache_get( 'psmt_gallery_media_' . $media_id, 'psmt' );
}
 ...
 */
function psmt_add_media_to_cache( $media ) {
	wp_cache_set( 'psmt_gallery_media_' . $media->id, $media, 'psmt' );
}
 ...
 * @param int $media_id media id.
 */
function psmt_delete_media_cache( $media_id ) {
	...
	if ( psmt_get_media_from_cache( $media_id ) ) {
		wp_cache_delete( 'psmt_gallery_media_' . $media_id, 'psmt' );
	}
}
