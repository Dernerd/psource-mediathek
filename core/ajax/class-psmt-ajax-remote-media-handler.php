<?php
/**
 * PsourceMediathek Remote Media Handler
 *
 ...
 */
class PSMT_Ajax_Remote_Media_Handler {
	...
	public function setup() {
		add_action( 'wp_ajax_psmt_add_remote_media', array( $this, 'add_remote_media' ) );
	}
	...
	public function add_remote_media() {
		...
		// Is remote enabled?
		if ( ! psmt_is_remote_enabled() ) {
			wp_send_json_error( array(
				'message' => __( 'Invalid action.', 'psourcemediathek' ),
			) );
		}
		...
		if ( ! $url ) {
			wp_send_json_error( array(
				'message' => __( 'Please provide a valid url.', 'psourcemediathek' ),
			) );
		}
		...
		if ( ! psmt_is_enabled( $component, $component_id ) ) {
			wp_send_json_error( array(
				'message' => __( 'Sorry, the functionality is disabled temporarily.', 'psourcemediathek' ),
			) );
		}
		$remote_args = array();
		$size_info = psmt_get_media_size( 'large' );
		$remote_args['width'] = isset( $size_info['width'] ) ? $size_info['width'] : 600;

		$parser = new PSMT_Remote_Media_Parser( $url, $remote_args );

		// It is neither raw url, nor oembed, we can not handle it.
		if ( ! $parser->is_raw && ! $parser->is_oembed ) {
			wp_send_json_error( array(
				'message' => __( 'Sorry, can not add the url.', 'psourcemediathek' ),
			) );
		}

		$type = $parser->type;
		// Invalid media type?
		if ( ! $type || ! psmt_component_supports_type( $component, $type ) ) {
			wp_send_json_error( array( 'message' => __( 'This type is not supported.', 'psourcemediathek' ) ) );
		}
		...
		if ( $gallery && empty( $gallery->type ) ) {
			...
			psmt_update_gallery_type( $gallery, $type );
		}
		...
		if ( ! psmt_is_mixed_gallery( $gallery ) && $type !== $gallery->type ) {
			// if we are uploading to a gallery and It is not a mixed gallery, the media type must match the gallery type.
			wp_send_json_error( array(
				'message' => sprintf( __( 'This type is not allowed in current gallery. Only <strong>%s</strong> is allowed!', 'psourcemediathek' ), psmt_get_type_singular_name( $type ) ),
			) );
		}
		...
		if ( ! psmt_user_can_add_remote_media( $component, $component_id, $gallery ) ) {

			$error_message = apply_filters( 'psmt_remote_media_add_permission_denied_message', __( "You don't have sufficient permissions.", 'psourcemediathek' ), $component, $component_id, $gallery );
		...
		wp_send_json_success( array(
			...
			'thumb_url'   => psmt_get_media_src( 'thumbnail', $id ),
			'is_remote'   => $is_remote,
			'is_raw'      => isset( $info['is_raw'] ) ? $info['is_raw'] : 0,
			'is_oembed'   => $is_oembed,
			'remote_type' => $is_oembed ? 'oembed' : 'raw',
			'html'        => $is_oembed ? psmt_get_oembed_content( $id, 'thumbnail' ) : $info['html'],
			'permalink'   => psmt_get_media_permalink( $id ),
		) );
	}
	 ...
	 *
	 * @param PSMT_Remote_Media_Parser $parser importer.
	 * @param int                     $gallery_id gallery id.
	 *
	 * @return array|WP_Error
	 */
	private function handle_oembed( PSMT_Remote_Media_Parser $parser, $gallery_id ) {

		if ( ! $parser || ! $parser->data ) {
			return new WP_Error( 'invalid_embed', __( 'Invalid media.', 'psourcemediathek' ) );
		}

		if ( ! psmt_is_oembed_enabled() ) {
				return new WP_Error( 'not_supported', __( 'Not supported.', 'psourcemediathek' ) );
		}
		...
		switch ( $parser->type ) {
			...
			case 'photo':
				...
				if ( ! $html ) {
					return new WP_Error( 'no_data', __( 'There was an issue, please try again.', 'psourcemediathek' ) );
				}
		...
		if ( empty( $info['title'] ) ) {
			$info['title'] = ! empty( $parser->data->author_name ) ? $parser->data->author_name : __( 'Video', 'psourcemediathek' );
		}
		return $info;
	}
	 ...
	 * @return array|WP_Error
	 */
	private function handle_raw( PSMT_Remote_Media_Parser $parser, $gallery_id ) {

		if ( ! psmt_is_remote_file_enabled() ) {
			return new WP_Error( 'not_supported', __( 'Not supported.', 'psourcemediathek' ) );
		}
	...
	private function process_media( $url, $args ) {
		...
		$download_remote_media = psmt_is_remote_file_download_enabled();
		// get the uploader.
		...
		if ( $download_remote_media && ! psmt_has_available_space( $gallery->component, $gallery->component_id ) ) {
			wp_send_json_error( array(
