<?php
/**
 * PsourceMediathek Media Only Activity Post Handler.
 *
 ...
 */
class PSMT_Ajax_Activity_Post_Handler {

	/**
	 * Singleton instance.
	 *
	 * @var PSMT_Ajax_Activity_Post_Handler
	 */
	...
	public function activity_post_update() {
		...
		if ( ! empty( $content ) || empty( $_POST['psmt-attached-media'] ) ) {
		...
		if ( false === $activity_id ) {
			exit( '-1<div id="message" class="error bp-ajax-message"><p>' . __( 'There was a problem posting your update. Please try again.', 'psourcemediathek' ) . '</p></div>' );
		...
		if ( function_exists( 'bp_nouveau_ajax_post_update' ) ) {
			...
			wp_send_json_success( array(
				'id'           => $activity_id,
				'message'      => esc_html__( 'Update posted.', 'psourcemediathek' ) . ' ' . sprintf( '<a href="%s" class="just-posted">%s</a>', esc_url( bp_activity_get_permalink( $activity_id ) ), esc_html__( 'View activity.', 'psourcemediathek' ) ),
				'activity'     => $content,
	...
	private function _post_update( $args = array() ) {
		return psmt_activity_post_update( $args );
	}
	...
	private function _groups_post_update( $args = array() ) {
		return psmt_activity_post_group_update( $args );
	}
