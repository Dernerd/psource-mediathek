<?php
/**
 * PsourceMediathek AddMedia/Cover handler.
 *
 ...
 * For now, It is the monolithic implementation for most of the psourcemediathek actions
 */
class PSMT_Ajax_Helper {

	/**
	 * Singleton instance.
	 *
	 * @var PSMT_Ajax_Helper
	 */
	...
	private function setup() {
		...
		add_action( 'wp_ajax_psmt_add_media', array( $this, 'add_media' ) );
		add_action( 'wp_ajax_psmt_upload_cover', array( $this, 'cover_upload' ) );
	}
	...
	public function add_media() {
		...
		check_ajax_referer( 'psmt_add_media' );

		$response = array();

		$file = $_FILES;

		// input file name, set via the psmt.Uploader
		...
		$context      = psmt_get_upload_context( false, $context );
		...
		// Check if PsourceMediathek is enabled for this component/component id.
		if ( ! psmt_is_enabled( $component, $component_id ) ) {
			wp_send_json_error( array(
				'message' => __( 'Sorry, the upload functionality is disabled temporarily.', 'psourcemediathek' ),
			) );
		}
		...
		$uploader = psmt_get_storage_manager();

		// check if the server can handle the upload?
		if ( ! $uploader->can_handle() ) {
			wp_send_json_error( array(
				'message' => __( 'Server can not handle this much amount of data. Please upload a smaller file or ask your server administrator to change the settings.', 'psourcemediathek' ),
			) );
		}

		// check if the user has available storage for his profile
		// or the component gallery(component could be groups, sitewide).
		if ( ! psmt_has_available_space( $component, $component_id ) ) {
			wp_send_json_error( array(
				'message' => __( 'Unable to upload. You have used the allowed storage quota!', 'psourcemediathek' ),
			) );
		}
		...
		$media_type = psmt_get_media_type_from_extension( psmt_get_file_extension( $file[ $file_id ]['name'] ) );

		// Invalid media type?
		if ( ! $media_type || ! psmt_component_supports_type( $component, $media_type ) ) {
			wp_send_json_error( array( 'message' => __( 'This file type is not supported.', 'psourcemediathek' ) ) );
		}
		...
		if ( $gallery && empty( $gallery->type ) ) {
			...
			psmt_update_gallery_type( $gallery, $media_type );
		}
		...
		if ( ! $gallery ) {
			...
			$gallery = psmt_get_context_gallery( array(
				'component'    => $component,
		...
		if ( ! $gallery ) {
			wp_send_json_error( array( 'message' => __( 'The gallery is not selected.', 'psourcemediathek' ) ) );
		}

		// if we are here, It means we have found a gallery to upload
		// check if gallery has a valid status?
		$is_valid_status = psmt_is_active_status( $gallery->status );
		if ( ! $is_valid_status ) {
			$default_status = psmt_get_default_status();
			// Check and update status if applicable.
			if (  psmt_is_active_status( $default_status ) && psmt_component_supports_status( $component, $default_status ) ) {
				// the current gallery status is invalid,
				// update status to current default privacy.
				psmt_update_gallery_status( $gallery, $default_status );
				$gallery->status = $default_status;
			} else {
				// should we inform user that we can't handle this request due to status issue?
				wp_send_json_error( array( 'message' => __( 'There was a problem with the privacy of your gallery.', 'psourcemediathek' ) ) );
			}
		}
		...
		if ( ! psmt_is_mixed_gallery( $gallery ) && $media_type !== $gallery->type ) {
			// if we are uploading to a gallery and It is not a mixed gallery, the media type must match the gallery type.
			wp_send_json_error( array(
				'message' => sprintf( __( 'This file type is not allowed in current gallery. Only <strong>%s</strong> files are allowed!', 'psourcemediathek' ), psmt_get_allowed_file_extensions_as_string( $gallery->type ) ),
			) );
		}
		...
		if ( $gallery ) {
			...
			if ( ! empty( $gallery->component ) && psmt_is_active_component( $gallery->component ) ) {
		...
		if ( ! psmt_user_can_upload( $component, $component_id, $gallery ) ) {

			$error_message = apply_filters( 'psmt_upload_permission_denied_message', __( "You don't have sufficient permissions to upload.", 'psourcemediathek' ), $component, $component_id, $gallery );
		...
		if ( ! psmt_is_active_status( $status ) || ! psmt_component_supports_status( $component, $status ) ) {
			// The status must be valid and supported by current component.
			// else we won't process upload.
			wp_send_json_error( array( 'message' => __( 'There was a problem with the privacy.', 'psourcemediathek' ) ) );
		}
		...
		if ( ! isset( $uploaded['error'] ) ) {
			...
			if ( apply_filters( 'psmt_use_processed_file_name_as_media_title', false ) ) {
			...
			// Orphan means not associated with the psourcemediathek unless the activity to which it was attached is actually created,
			...
			if (  'activity' !== $context && ! psmt_is_auto_publish_to_activity_enabled( 'add_media' ) ) {
				psmt_gallery_add_unpublished_media( $gallery_id, $id );
			}

			psmt_gallery_increment_media_count( $gallery_id );

			$attachment = psmt_media_to_json( $id );
	...
	public function cover_upload() {
		...
		$parent_id   = absint( $_POST['psmt-parent-id'] );
		$parent_type = isset( $_POST['psmt-parent-type'] ) ? trim( $_POST['psmt-parent-type'] ) : 'gallery';
		...
		if ( $media_type != 'photo' ) {
			...
			wp_send_json( array(
				'message' => sprintf( __( 'Please upload a photo. Only <strong>%s</strong> files are allowed!', 'psourcemediathek' ), psmt_get_allowed_file_extensions_as_string( $media_type ) )
			) );
		}
		...
		if ( ! psmt_user_can_upload( $component, $component_id, $gallery ) ) {
			...
			wp_send_json_error( array( 'message' => __( "You don't have sufficient permissions to upload.", 'psourcemediathek' ) ) );
		}
		...
		if ( ! isset( $uploaded['error'] ) ) {
			...
			add_filter( 'psmt_do_not_record_add_media_activity', '__return_true' );

			$id = psmt_add_media( $media_data );
			// in case media creation failed.
			if ( ! $id ) {
				wp_send_json_error( array( 'message' => __( 'There was a problem. Please try again.', 'psourcemediathek' ) ) );
			}

			if ( $parent_type == 'gallery' ) {
				$old_cover = psmt_get_gallery_cover_id( $parent_id );

			} else {
				$old_cover = psmt_get_media_cover_id( $parent_id );
			}
			...
			if ( $gallery->type == 'photo' ) {
				...
				psmt_delete_media_meta( $id, '_psmt_is_psmt_media' );

				if ( $old_cover ) {
					psmt_delete_media( $old_cover );
				}
			}

			psmt_update_media_cover_id( $parent_id, $id );
