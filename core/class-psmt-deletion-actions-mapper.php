<?php
/**
 * Handles mapping of various WordPress delete actions to PsourceMediathek.
 *
...
/**
 * Maps various deletion actions of WordPress to PsourceMediathek in order to provide clear and consistent api for PsourceMediathek plugins
 *
 ...
 */
class PSMT_Deletion_Actions_Mapper {
	...
	private function __construct() {
		...
		add_filter( 'psmt_cleanup_single_media_on_delete', array(
			$this,
	 ...
	 *
	 * @return PSMT_Deletion_Actions_Mapper
	 */
	...
	/**
	 * Maps the before_delete_post action to psmt_before_gallery_delete action
	...
	public function map_before_delete_post_action( $gallery_id ) {
		...
		if ( ! $gallery_id || ! psmt_is_valid_gallery( $gallery_id ) ) {
			return;
		}
		...
		// fire the PsourceMediathek gallery delete action.
		...
		if ( ! empty( $media_ids ) ) {
			$mid             = $media_ids[0];
			$storage_manager = psmt_get_storage_manager( $mid );
		}
		...
		if ( psourcemediathek()->is_bp_active() ) {
			...
			psmt_gallery_delete_activity( $gallery_id );
			// delete all associated activity meta
			// psmt_gallery_delete_activity_meta( $gallery_id );
		}

		// Delete wall gallery meta.
		psmt_delete_wall_gallery_id( array(
	...
	/**
	 * Maps 'delete_attachment' to psmt_before_media_delete action
	...
	public function map_before_delete_attachment_action( $media_id ) {
		...
		if ( ! psmt_is_valid_media( $media_id ) ) {
		...
		/**
		 * Action 'psmt_before_media_delete' fires before WordPress starts deleting an attachment which is a valid media( in PsourceMediathek).
		 * Any plugin utilizing this action can access the fully functional media object by using psmt_get_media() on the passed media id
		 */
		do_action( 'psmt_before_media_delete', $media_id );

		$storage_manager = psmt_get_storage_manager( $media_id );
		$storage_manager->delete_media( $media_id );

		// delete if it is set as cover.
		delete_metadata( 'post', null, '_psmt_cover_id', $media_id, true ); // delete all for any posts.
		delete_metadata( 'post', null, '_psmt_unpublished_media_id', $media_id, true ); // delete all for any posts.
		...
		if ( psmt_media_has_cover_image( $media_id ) ) {
			psmt_delete_media( psmt_get_media_cover_id( $media_id ) );
		}

		if ( apply_filters( 'psmt_cleanup_single_media_on_delete', true, $media_id, $gallery_id ) ) {
			psmt_gallery_decrement_media_count( $gallery_id );

			if ( psourcemediathek()->is_bp_active() ) {
				// delete all activities related to this media.
				// psmt_media_delete_activities( $media_id );
				psmt_delete_activity_for_single_published_media( $media_id );
				// delete all activity meta key where this media is associated.
				psmt_media_delete_activity_meta( $media_id );
			}
		}
	...
	/**
	 * Maps the 'deleted_post' action to 'psmt_gallery_deleted' or 'psmt_media_deleted' action depending on
	 ...
	 * CAUTION: do not try to call psmt_get_media on this action, may give bad results
	 *
	 * @param int $media_id media id.
	 */
	private function do_media_deleted( $media_id ) {
		// clear media cache too?
		do_action( 'psmt_media_deleted', $media_id );
	}
...
PSMT_Deletion_Actions_Mapper::get_instance();
