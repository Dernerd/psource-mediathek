<?php
/**
 * PsourceMediathek Gallery query
 *
 ...
 */
class PSMT_Gallery_Query extends WP_Query {
	...
	public function query( $args ) {
		...
		// the should do it on the action 'psmt_before_gallery_query'.
		do_action( 'psmt_before_gallery_query', $this, $args );

		$posts = parent::query( $args );
		// if you need to do some processing after the query has finished.
		do_action( 'psmt_after_gallery_query', $this, $args );
	...
	public function build_params( $args ) {
		...
		$defaults = array(
			...
			'per_page'      => psmt_get_option( 'galleries_per_page' ),
			'offset'        => false,
	...
	public function the_gallery() {
		...
		if ( $this->current_post === - 1 ) {
			...
			do_action_ref_array( 'psmt_gallery_loop_start', array( &$this ) );
		}
		...
		psourcemediathek()->current_gallery = psmt_get_gallery( $post );

		psmt_setup_gallery_data( $post );
	}
	...
	public function is_main_query() {
		...
		return $this === $psourcemediathek->the_gallery_query;
	}
	...
	public function reset_gallery_data() {
		...
		if ( ! empty( $this->post ) ) {
			psourcemediathek()->current_gallery = psmt_get_gallery( $this->post );
		}
	}
	...
	public function paginate( $default = true ) {
		...
		if ( $total > 1 ) {
			...
			if ( ! defined('DOING_AJAX' ) ) {
				...
				$link = psmt_get_current_page_uri();
	...
	public function pagination_count() {
		...
		printf( __( 'Viewing gallery %d to %d (of %d galleries)', 'psourcemediathek' ), $from_num, $to_num, $this->found_posts );
	}
	...
	/**
	 * Build this PSMT_Gallery_Query from WP_Query object
	 *
	 * @param WP_Query $wp_query the WP_Query object.
	 *
	 * @return PSMT_Gallery_Query
	 */
 ...
 * @return bool
 */
function psmt_setup_gallery_data( $post ) {
 ...
 */
function psmt_reset_gallery_data() {

	if ( psourcemediathek()->the_gallery_query ) {
		psourcemediathek()->the_gallery_query->reset_gallery_data();
	}
 ...
 */
function _psmt_cache_gallery_cover( $query ) {
	...
	foreach ( (array) $gallery_ids as $gallery_id ) {
		...
		$media_id = psmt_get_gallery_cover_id( $gallery_id );
...
add_action( 'psmt_gallery_loop_start', '_psmt_cache_gallery_cover' );
