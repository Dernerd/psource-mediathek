...
class PSMT_Menu {
	...
	public function add_item( $args ) {
		...
		$default = array(
			...
			'action'   => '',
			'url'      => '',
			'callback' => 'psmt_is_menu_item_visible', // we might have used capability here.
		);
	...
	public function render( $gallery, $selected = '' ) {
		...
		$items = apply_filters( 'psmt_pre_render_gallery_menu_items', $this->items, $gallery );
		...
		foreach ( $items as $item ) {
			...
			if ( is_callable( $item['callback'] ) && call_user_func( $item['callback'], $item, $gallery ) ) {
				...
				if ( $item['action'] === $selected ) {
					$class = 'psmt-selected-item';
				}

				$html[] = sprintf( '<li><a href="%1$s" title ="%2$s" id="%3$s" data-psmt-action ="%4$s" class="%6$s">%5$s</a></li>', $item['url'], $item['label'], 'psmt-gallery-menu-item-' . $item['slug'], $item['action'], $item['label'], $class );
			}
		}
 ...
 */
class PSMT_Gallery_Menu extends PSMT_Menu {
}

/**
 * Media context menu class.
 */
class PSMT_Media_Menu extends PSMT_Menu {
