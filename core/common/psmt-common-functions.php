 ...
 * @return string
 */
function psmt_get_gallery_post_type() {
	/**
	 ...
	 */
	// apply_filters( 'psmt_gallery_post_type',  );.
	return 'psmt-gallery';
}
 ...
 * @return string
 */
function psmt_get_gallery_post_type_rewrite_slug() {

	$slug = psmt_get_option( 'gallery_permalink_slug' );

	if ( ! $slug ) {
		$slug = 'gallery';
	}

	return apply_filters( 'psmt_gallery_post_type_rewrite_slug', $slug );
}
 ...
 * @return string
 */
function psmt_get_media_post_type() {
	return 'attachment';
}
 ...
 * @return string
 */
function psmt_get_type_taxname() {
	return 'psmt-type';
}
 ...
 * @return string
 */
function psmt_get_component_taxname() {
	return 'psmt-component';
}
 ...
 * @return string
 */
function psmt_get_status_taxname() {
	return 'psmt-status';
}

/**
 * Check if PsourceMediathek is enabled for the given component
 *
 ...
 * @return boolean
 */
function psmt_is_enabled( $component, $component_id ) {
	return apply_filters( 'psmt_is_enabled', psmt_is_active_component( $component ), $component, $component_id );
}

/**
 * Get an array of all the PsourceMediathek taxonomy info
 *
 * @return array
 */
function psmt_get_all_taxonomies_info() {
	return psourcemediathek()->taxonomies;
}

/**
 * Is PsourceMediathek network activated on the multisite install?
 *
 * @return boolean
 */
function psmt_is_network_activated() {
	...
	if ( isset( $plugins[ psourcemediathek()->get_basename() ] ) ) {
		return true;
	}
 ...
 */
function psmt_post_exists( $args ) {
	...
	$posts = get_posts(
		array(
			...
			'name'                      => $args['slug'],
			psmt_get_component_taxname() => psmt_underscore_it( $args['component'] ),
			'meta_query'                => array(
				array(
 ...
 */
function psmt_term_exists( $term, $taxonomy = '', $parent = 0 ) {

	$term = psmt_strip_underscore( $term );

	return _psmt_get_term( $term, $taxonomy );
}
 ...
 */
function psmt_underscore_it( $str ) {
 ...
 */
function psmt_strip_underscore( $str ) {
 ...
 * @return array
 */
function psmt_string_to_array( $string, $delim = ',' ) {
 ...
 *
 * Example psmt_array_to_string( array( 'a', 'b', 'c' ) return "a,b,c"
 *
 ...
 */
function psmt_array_to_string( $array, $delim = ',' ) {
 ...
 * @return string
 */
function psmt_get_current_page_uri() {
 ...
 *
 * @param int|PSMT_Media|PSMT_Gallery $object media|gallery id or object.
 * @param string                    $taxonomy taxonomy name.
 *
 * @return boolean|int false or term id
 */
function _psmt_get_object_term_id( $object, $taxonomy ) {
 ...
 *
 * @param mixed|PSMT_Gallery|PSMT_Media $object object or ID.
 *
 * @return int status ID(term id for the status associated with this post)
 */
function psmt_get_object_status_term_id( $object ) {
	return _psmt_get_object_term_id( $object, psmt_get_status_taxname() );
}

/**
 * Get the slug of the status term( e.g private|public|friendsonly etc)
 *
 * @param int|PSMT_Media|PSMT_Gallery $object media or gallery object or id.
 *
 * @return string status slug(used to uniquely identify this status) e.g (private|public|friends)
 */
function psmt_get_object_status( $object ) {

	$status_id = psmt_get_object_status_term_id( $object );

	// now get the slug for this status.
	$status = psmt_get_status_object( $status_id );
	...
	return psmt_strip_underscore( $slug );
 ...
 *
 * @return string label for the status e.g (Private| Public| Nur Freunde)
 */
function psmt_get_object_status_label( $object ) {
 ...
 */
function psmt_get_object_type_term_id( $object ) {
	return _psmt_get_object_term_id( $object, psmt_get_type_taxname() );
}

/**
 * Get the object type(photo|video etc)
 *
 * @param int|PSMT_Gallery|PSMT_Media $object media/gallery object or id.
 *
 * @return string type slug(used to uniquely identify this status) e.g (audio|video|photo)
 */
function psmt_get_object_type( $object ) {

	$type_id = psmt_get_object_type_term_id( $object );

	// now get the slug for this status.
	$type = psmt_get_type_object( $type_id );
	$slug = '';
 ...
 *
 * @param int|PSMT_Gallery|PSMT_Media $object gallery or media object or ID.
 *
 * @return string label for the gallery, media type e.g (Audio| Video| Photo)
 */
function psmt_get_object_type_label( $object ) {
 ...
 * @return string label for the gallery, media type e.g (Audio| Video| Photo)
 */
function psmt_get_object_type_singular_name( $object ) {
 ...
 */
function psmt_get_object_type_plural_name( $object ) {
 ...
 */
function psmt_get_object_component_term_id( $object ) {
	return _psmt_get_object_term_id( $object, psmt_get_component_taxname() );
}
 ...
 */
function psmt_get_object_component( $object ) {

	$term_id = psmt_get_object_component_term_id( $object );

	$slug = '';
	// now get the slug for this status.
	$component = psmt_get_component_object( $term_id );
 ...
 */
function psmt_get_object_component_label( $object ) {

	$component_term_id = psmt_get_object_component_term_id( $object );

	// now get the slug for this component.
	$component = psmt_get_component_object( $component_term_id );
 ...
 * @return int term id
 */
function psmt_get_type_term_id( $type ) {
	return psmt_get_term_id_by_slug( $type, 'types' );
}
 ...
 * @return int
 */
function psmt_get_status_term_id( $status ) {
	return psmt_get_term_id_by_slug( $status, 'statuses' );
}
 ...
 * @return int
 */
function psmt_get_component_term_id( $component ) {
	return (int) psmt_get_term_id_by_slug( $component, 'components' );
}
 ...
 * @return int
 */
function psmt_get_current_component_id( $component_id = null ) {

	$owner_id = 0;

	if ( psmt_is_sitewide_gallery_component() ) {
	...
	return apply_filters( 'psmt_get_current_component_id', $owner_id ); // context sensitive dd.
 ...
 */
function psmt_get_current_component() {

	if ( isset( $_POST['_psmt_current_component'] ) ) {
		$component = trim( $_POST['_psmt_current_component'] );

		if ( ! psmt_is_active_component( $component ) ) {
			$component = '';
		}
	} elseif ( ! psourcemediathek()->is_bp_enabled() || psmt_is_sitewide_gallery_component() ) {
	...
	return strtolower( apply_filters( 'psmt_get_current_component', $component ) ); // context sensitive.
 ...
 * @return bool
 */
function psmt_admin_is_edit_gallery() {
 ...
 * @return bool
 */
function psmt_admin_is_add_gallery() {

	if ( is_admin() && isset( $_GET['post_type'] ) && psmt_get_gallery_post_type() === $_GET['post_type'] && isset( $_GET['psmt-gallery-type'] ) ) {
		return true;
	}
 ...
 * @return array
 */
function psmt_get_reserved_slugs() {
	...
	$reserved = array_merge( $reserved, array_keys( psmt_get_registered_components() ), array_keys( psmt_get_registered_statuses() ), array_keys( psmt_get_registered_types() ) );

	return apply_filters( 'psmt_reserved_slugs', $reserved );
 ...
 * @return boolean
 */
function psmt_is_reserved_slug( $slug ) {

	$reserved = psmt_get_reserved_slugs();
...
/**
 * Update Media extensions, save it to psmt-settings
 *
 * @param string $type media type.
 * @param array  $extensions array of file extensions.
 */
function psmt_update_media_extensions( $type, $extensions ) {

	$all_extensions = psmt_get_all_media_extensions();

	$all_extensions = array_map( 'psmt_array_to_string', $all_extensions );

	if ( ! empty( $extensions ) ) {
		$all_extensions[ $type ] = join( ',', $extensions );
	}

	psmt_update_option( 'extensions', $all_extensions );
 ...
 */
function psmt_get_media_extensions( $type ) {

	$extensions = psmt_get_all_media_extensions();
 ...
 */
function psmt_get_all_media_extensions() {

	$all_extensions = (array) psmt_get_option( 'extensions', array() );
	...
	foreach ( $all_extensions as $type => $extension ) {
		...
		$extensions[ $type ] = array_map( 'trim', psmt_string_to_array( strtolower( $extension ) ) );
	}
 ...
 * @return array
 */
function psmt_get_default_options() {
	$default = array(
		...
		'psmt_upload_space'                      => 10,
		'psmt_upload_space_groups'               => 10,
		// how many Mbs?
		...
		'gallery_archive_slug'                  => 'seiten-galerien',
		'gallery_permalink_slug'                => 'seiten-galerie',
 ...
 */
function psmt_get_all_options() {

	$defaults = psmt_get_default_options();
	$options = get_option( 'psmt-settings', $defaults );

	return apply_filters( 'psmt_settings', $options );
}

/**
 * Save  all options
 *
 * @param array $options psourcemediathek options.
 *
 * @return mixed
 */
function psmt_save_options( $options ) {

	$callback = 'update_option';

	return $callback( 'psmt-settings', $options );
 ...
 */
function psmt_get_option( $option, $default = '' ) {


	$options = psmt_get_all_options();
...
/**
 * Update individual PsourceMediathek option and save that to database( in options table )
 *
 * @param string $option_name name of the option.
 * @param mixed  $value value of the option.
 */
function psmt_update_option( $option_name, $value ) {

	$options                 = psmt_get_all_options();
	$options[ $option_name ] = $value;

	psmt_save_options( $options );
}
 ...
 *        headers. Default: 302.
 */
function psmt_redirect( $location, $status = 302 ) {
 ...
 * @return string
 */
function psmt_get_default_status() {
	return psmt_get_option( 'default_status', 'public' );
}
 ...
 * @return PSMT_Status[]
 */
function psmt_get_active_statuses() {

	$active_status_keys  = (array) psmt_get_option( 'active_statuses' );
 ...
 * @return boolean
 */
function psmt_is_active_status( $status ) {
 ...
 * @return boolean
 */
function psmt_are_active_statuses( $statuses ) {
	...
	$valid_statuses = psmt_get_active_statuses();
 ...
 */
function psmt_get_active_components() {

	$registered_components = psmt_get_registered_components();

	$active_components_keys = (array) psmt_get_option( 'active_components' );
 ...
 * @return boolean
 */
function psmt_is_active_component( $component ) {
 ...
 * @return boolean
 */
function psmt_are_active_components( $components ) {
	...
	$valid_components = psmt_get_active_components();
 ...
 * @return PSMT_Type[]
 */
function psmt_get_active_types() {

	$active_type_keys = (array) psmt_get_option( 'active_types' );
	$registered_types = psmt_get_registered_types();
 ...
 * @return boolean
 */
function psmt_is_active_type( $type ) {

	if ( empty( $type ) ) {
		return false;
	}

	$valid_types = psmt_get_active_types();
 ...
 * @return boolean
 */
function psmt_are_active_types( $types ) {
 ...
 * @return string
 */
function psmt_get_type_singular_name( $type ) {
 ...
 * @return string
 */
function psmt_get_type_plural_name( $type ) {
 ...
 */
function psmt_get_selected_single_media_size() {
	return psmt_get_option( 'single_media_size', 'large' );
}
 ...
 * @return string size name(large|mid|thumbnail|original)
 */
function psmt_get_selected_lightbox_media_size() {
	return psmt_get_option( 'lightbox_media_size', 'large' );
}
 ...
 */
function psmt_component_init_type_support( $component ) {

	$supported_types = psmt_component_get_supported_types( $component );

	foreach ( $supported_types as $type ) {
		psmt_component_add_type_support( $component, $type );
	}
 ...
 * @return boolean
 */
function psmt_component_has_type_filters_enabled( $component, $component_id ) {
	return psmt_get_option( $component . '_enable_type_filters', 0 );
}
 ...
 * @return boolean
 */
function psmt_is_auto_publish_to_activity_enabled( $action ) {

	$enabled_types = psmt_get_option( 'autopublish_activities' );
 ...
 */
function psmt_get_html_attributes( $args = array() ) {
 ...
 */
function psmt_get_grid_column_class( $col ) {
	...
	if ( ! in_array( $col, $supported ) ) {
		return 'psmt-col-' . $col . ' psmt-col-not-supported';
	}
 ...
 * @return bool
 */
function psmt_recursive_delete_dir( $dir ) {
	...
	foreach ( $items as $item ) {
		...
		if ( is_dir( $item ) ) {
			psmt_recursive_delete_dir( $file );
		} else {
			@ unlink( $file );
		}
	}
 ...
 * @return string
 */
function psmt_get_dynamic_user_id_for_context( $context ) {
