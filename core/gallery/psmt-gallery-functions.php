 ...
 */
function psmt_get_registered_statuses() {

	$psourcemediathek = psourcemediathek();

	return $psourcemediathek->gallery_statuses;
}
 ...
 * @return boolean
 */
function psmt_is_registered_status( $status ) {
 ...
 * @return boolean
 */
function psmt_are_registered_statuses( $statuses ) {
	...
	$statuses = psmt_string_to_array( $statuses );

	$valid_statuses = psmt_get_registered_statuses();
 ...
 *
 * @return PSMT_Component[] keys are $component_name(groups|members etc)
 */
function psmt_get_registered_components() {

	$psourcemediathek = psourcemediathek();

	return $psourcemediathek->components;
}

/**
 *  Is valid gallery associated component
 *
 * @deprecated use psmt_is_registered_component
 *
 * @param string $component component name ( members|groups etc ).
 *
 * @return boolean
 */
function psmt_is_registered_gallery_component( $component ) {
	return psmt_is_registered_component( $component );
}
 ...
 * @return boolean
 */
function psmt_is_registered_component( $component ) {

	if ( empty( $component ) ) {
		return false;
	}

	$valid_components = psmt_get_registered_components();
 ...
 * @return boolean
 */
function psmt_are_registered_components( $components ) {

	if ( empty( $components ) ) {
		return false;
	}

	$components = psmt_string_to_array( $components );
 ...
 * @return PSMT_Type[]
 */
function psmt_get_registered_types() {

	$psourcemediathek = psourcemediathek();

	return $psourcemediathek->types;
}
 ...
 * @return boolean
 */
function psmt_is_registered_type( $type ) {
 ...
 * @return boolean
 */
function psmt_are_registered_types( $types ) {

	if ( empty( $types ) ) {
		return false;
	}

	$types = psmt_string_to_array( $types );
 ...
 * @return boolean
 */
function psmt_is_valid_gallery( $id ) {
	return get_post_type( $id ) === psmt_get_gallery_post_type();
}
 ...
 * @return boolean
 */
function psmt_is_sitewide_gallery( $id ) {

	if ( ! psmt_is_valid_gallery( $id ) ) {
		return false;
	}
	// assume it is a valid gallery.
	$gallery = psmt_get_gallery( $id );
 ...
 * @return int
 */
function psmt_get_wall_gallery_id( $args ) {
	...
	$id        = 0;
	$func_name = "psmt_get_{$component}_wall_{$media_type}_gallery_id";

	if ( function_exists( $func_name ) ) {
		$id = call_user_func( $func_name, $component_id );
	}

	return apply_filters( 'psmt_get_wall_gallery_id', $id, $component_id, $component, $media_type );
}
 ...
 * @return boolean
 */
function psmt_update_wall_gallery_id( $args ) {
	...
	$func_name = "psmt_update_{$component}_wall_{$media_type}_gallery_id";

	if ( function_exists( $func_name ) ) {
		$id = call_user_func( $func_name, $component_id, $gallery_id );
	}

	return apply_filters( 'psmt_update_wall_gallery_id', $id, $component_id, $component, $media_type );
}
 ...
 * @return boolean
 */
function psmt_delete_wall_gallery_id( $args ) {
	...
	$func_name = "psmt_delete_{$component}_wall_gallery_id";
	...
	do_action( 'psmt_delete_wall_gallery_id', $id, $component_id, $component, $media_type );
 ...
 *
 * @param int|PSMT_Gallery $gallery_id numeric gallery id or gallery object.
 *
 * @return boolean true if wall gallery, false if not wall gallery
 */
function psmt_is_wall_gallery( $gallery_id ) {
	...
	$wall_gallery_id = psmt_get_wall_gallery_id( array(
 ...
 * @return int
 */
function psmt_get_members_wall_photo_gallery_id( $user_id ) {
	return (int) psmt_get_user_meta( $user_id, '_psmt_wall_photo_gallery_id', true );
}
 ...
 * @return int
 */
function psmt_get_members_wall_video_gallery_id( $user_id ) {
	return (int) psmt_get_user_meta( $user_id, '_psmt_wall_video_gallery_id', true );
}
 ...
 * @return int
 */
function psmt_get_members_wall_audio_gallery_id( $user_id ) {
	return (int) psmt_get_user_meta( $user_id, '_psmt_wall_audio_gallery_id', true );
}
 ...
 * @return int|boolean
 */
function psmt_update_members_wall_photo_gallery_id( $user_id, $gallery_id ) {
	return psmt_update_user_meta( $user_id, '_psmt_wall_photo_gallery_id', $gallery_id );
}
 ...
 * @return int|boolean
 */
function psmt_update_members_wall_video_gallery_id( $user_id, $gallery_id ) {
	return psmt_update_user_meta( $user_id, '_psmt_wall_video_gallery_id', $gallery_id );
}
 ...
 * @return int|boolean
 */
function psmt_update_members_wall_audio_gallery_id( $user_id, $gallery_id ) {
	return psmt_update_user_meta( $user_id, '_psmt_wall_audio_gallery_id', $gallery_id );
}
 ...
 * @return int|boolean
 */
function psmt_get_members_wall_doc_gallery_id( $user_id ) {

	return (int) psmt_get_user_meta( $user_id, '_psmt_wall_doc_gallery_id', true );
}
 ...
 * @return int|boolean
 */
function psmt_update_members_wall_doc_gallery_id( $user_id, $gallery_id ) {

	return psmt_update_user_meta( $user_id, '_psmt_wall_doc_gallery_id', $gallery_id );
}
 ...
 * @return int|boolean
 */
function psmt_delete_members_wall_gallery_id( $user_id, $type, $gallery_id ) {

	$key = "_psmt_wall_{$type}_gallery_id";

	$wall_gallery_id = (int) psmt_get_user_meta( $user_id, $key, true );

	if ( psmt_delete_user_meta( $user_id, $key, $gallery_id ) ) {
 ...
 * @todo update docs.
 */
function psmt_create_gallery( $args = array() ) {
	...
	if ( isset( $args['id'] ) && ! empty( $args['id'] ) ) {
		return psmt_update_gallery( $args );
	}
	...
	$gallery_status = psmt_get_default_status();

	if ( ! empty( $status ) ) {

		if ( psmt_is_registered_status( $status ) ) {
			$gallery_status = $status;
		}
	}

	// Make sure that component supports the status and type.
	if ( ! psmt_component_supports_status( $component, $gallery_status )
	     || ! psmt_component_supports_type( $component, $type ) ) {
		return false;
	}

	// hierarchical tax should always pass ids.
	$tax = array(
		psmt_get_component_taxname() => psmt_underscore_it( $component ),
		psmt_get_type_taxname()      => psmt_underscore_it( $type ),
		psmt_get_status_taxname()    => psmt_underscore_it( $gallery_status ),
	);
	...
	psmt_update_gallery_meta( $gallery_id, '_psmt_component_id', $component_id );
	// fire the gallery created action.
	do_action( 'psmt_gallery_created', $gallery_id );
 ...
 */
function psmt_update_gallery( $args ) {

	if ( ! isset( $args['id'] ) ) {
		return new WP_Error( 0, __( 'Must provide ID to update a gallery.', 'psourcemediathek' ) );
	}
	...
	if ( ! $gallery ) {
		return new WP_Error( 0, __( 'Gallery Does not exist!', 'psourcemediathek' ) );
	}
	...
	$gallery_data = array(
		...
		'component_id' => psmt_get_gallery_meta( $gallery->ID, '_psmt_component_id' ),
		'component'    => '',
		'status'       => '',
		'type'         => '',
		'date_created' => '',
	...
	if ( $component && psmt_is_active_component( $component ) ) {
		$tax[ psmt_get_component_taxname() ] = psmt_underscore_it( $component );
	}

	if ( $type && psmt_is_active_type( $type ) ) {
		$tax[ psmt_get_type_taxname() ] = psmt_underscore_it( $type );
	}

	if ( $status && psmt_is_active_status( $status ) ) {
		$tax[ psmt_get_status_taxname() ] = psmt_underscore_it( $status );
	}
	...
	do_action( 'psmt_gallery_updated', $gallery_id );
 ...
 * Delete a Gallery
 *
 * For an outline of actions, please see PSMT_Deletion_Actions_Mapper class.
 * This function calls actions 'psmt_before_gallery_delete' -> deletes everything -> 'psmt_gallery_deleted'
 *
 ...
 */
function psmt_delete_gallery( $gallery_id, $force_delete = true ) {

	/**
	 *
	 * @see PSMT_Deletion_Actions_Mapper::map_before_delete_post_action()
	 * @see PSMT_Deletion_Actions_Mapper::map_deleted_post() for the approprivte function
	 */
	/**
	 * Action flow
	 *  wp_delete_post()
	 *        -> do_action('before_delete_post')
	 *        -> PSMT_Deletion_Actions_Mapper::map_before_delete_post_action()
	 *        -> do_action ( 'psmt_before_gallery_delete, $gallery_id )
	 *        -> cleanup gallery
	 *        .........
	 *        .........
	 *
	 ...
	 *        -> do_action( 'psmt_gallery_deleted', $gallery_id )
	 */
 ...
 * @param array $args array of args.
 *
 * @return mixed|PSMT_Gallery
 */
function psmt_get_context_gallery( $args ) {
	$defaults = array(
		'component'    => '',
		'component_id' => '',
		'context'      => '',
	...
	$gallery = apply_filters( 'psmt_get_context_gallery', $gallery, $args );
 ...
 *
 * @param int|PSMT_Gallery $gallery gallery to update.
 * @param string          $type ( audio|video|photo etc).
 */
function psmt_update_gallery_type( $gallery, $type ) {

	$gallery = psmt_get_gallery( $gallery );

	wp_set_object_terms( $gallery->id, psmt_get_type_term_id( $type ), psmt_get_type_taxname() );
}
 ...
 */
function psmt_update_gallery_status( $gallery, $status ) {

	$gallery = psmt_get_gallery( $gallery );

	wp_set_object_terms( $gallery->id, psmt_get_status_term_id( $status ), psmt_get_status_taxname() );
}
 ...
 */
function psmt_update_gallery_component( $gallery, $component ) {

	$gallery = psmt_get_gallery( $gallery );

	wp_set_object_terms( $gallery->id, psmt_get_component_term_id( $component ), psmt_get_component_taxname() );
}
 ...
 */
function psmt_gallery_exists( $gallery_slug_or_id, $component, $component_id ) {
 ...
 * @return int
 */
function psmt_get_total_gallery_for_user( $user_id = 0 ) {
	$needs_caching = false;
	$count         = 0;
	$context       = '';
	...
	if ( $context ) {
		$count = get_user_meta( $user_id, '_psmt_gallery_count_members_' . $blog_id . '_' . $context, true );
	}

	if ( ! $context || '' === $count ) {
		$needs_caching = true;
		$count         = psmt_get_gallery_count( array(
			'component'    => 'members',
	...
	if ( $context && $needs_caching ) {
		update_user_meta( $user_id, '_psmt_gallery_count_members_' . $blog_id . '_' . $context, $count );
	}
 ...
 * @return int
 */
function psmt_get_gallery_count_by_type( $type, $owner_type, $owner_id, $status = null ) {
	// check for the vailidity of the gallery type.
	if ( ! psmt_is_registered_type( $type ) ) {
		return 0; // that's not right.
	}

	return psmt_get_gallery_count( array(
 ...
 */
function psmt_get_public_gallery_count() {
 ...
 *
 * @see psmt_get_object_ids for parameters
 *
 * @return array of gallery IDS
 */
function psmt_get_gallery_ids( $args ) {
	return psmt_get_object_ids( $args, psmt_get_gallery_post_type() );
}
 ...
 * @param array $args array of args.
 *
 * @see psmt_get_object_count for parameter details
 * @return int
 *
 ...
 * We will do it when we introduce psmt-tools
 */
function psmt_get_gallery_count( $args ) {
	return psmt_get_object_count( $args, psmt_get_gallery_post_type() );
}
 ...
 * @return int
 */
function psmt_gallery_get_media_count( $gallery_id ) {
	return psmt_get_gallery_meta( $gallery_id, '_psmt_media_count', true );
}
 ...
 */
function psmt_gallery_update_media_count( $gallery_id, $count ) {
	return psmt_update_gallery_meta( $gallery_id, '_psmt_media_count', $count );
}
 ...
 * @return int|boolean meta id if not exists else true on success, false on failure.
 */
function psmt_gallery_increment_media_count( $gallery_id, $count = 1 ) {

	return psmt_gallery_change_media_count( $gallery_id, $count );
}
 ...
 * @return int|boolean meta id if not exists else true on success, false on failure.
 */
function psmt_gallery_decrement_media_count( $gallery_id, $count = - 1 ) {
 ...
 * @return int|boolean meta id if not exists else true on success, false on failure.
 */
function psmt_gallery_change_media_count( $gallery_id, $count = 1 ) {

	if ( ! $gallery_id ) {
		return false;
	}

	$current_count = absint( psmt_gallery_get_media_count( $gallery_id ) );
	...
	return psmt_update_gallery_meta( $gallery_id, '_psmt_media_count', $updated_count );
}
 ...
 * @return int|boolean meta id if not exists else true on success, false on failure.
 */
function psmt_mark_gallery_sorted( $gallery_id ) {
	return psmt_update_gallery_meta( $gallery_id, '_psmt_is_sorted', 1 );
}
 ...
 * @return boolean
 */
function psmt_is_gallery_sorted( $gallery_id ) {
	return psmt_get_gallery_meta( $gallery_id, '_psmt_is_sorted', true );
}
 ...
 */
function psmt_gallery_get_latest_media_id( $gallery_id ) {
 ...
 * @return string|null
 */
function psmt_gallery_breadcrumb( $args = null ) {
	...
	$name = '';
	if ( psourcemediathek()->is_bp_active() && bp_is_active( 'groups' ) && bp_is_group() ) {
		$name = bp_get_group_name( groups_get_current_group() );
	} elseif ( psourcemediathek()->is_bp_active() && bp_is_user() ) {
	...
	if ( $name ) {
		$my_or_his_gallery = sprintf( __( "%s Galerie", 'psourcemediathek' ), $name );
	}

	if ( function_exists( 'bp_is_my_profile' ) && bp_is_my_profile() ) {
		$my_or_his_gallery = __( 'Deine Galerien', 'psourcemediathek' );
	}

	if ( psmt_is_media_management() ) {
		$crumbs[] = sprintf( '<span>%s</span>', ucwords( psourcemediathek()->get_edit_action() ) );
	}
	...
	if ( psmt_is_single_media() ) {
		...
		if ( psmt_is_media_management() ) {
			$crumbs[] = sprintf( '<a href="%s">%s</a>', psmt_get_media_permalink( $media ), psmt_get_media_title( $media ) );
		} else {
			$crumbs[] = sprintf( '<span>%s</span>', psmt_get_media_title( $media ) );
		}
	}
	...
	if ( psmt_is_single_gallery() ) {
		...
		if ( psmt_is_gallery_management() || psmt_is_single_media() ) {
			$crumbs[] = sprintf( '<a href="%s">%s</a>', psmt_get_gallery_permalink( $gallery ), psmt_get_gallery_title( $gallery ) );
		} else {
			$crumbs[] = sprintf( '<span>%s</span>', psmt_get_gallery_title( $gallery ) );
		}
	}

	if ( $my_or_his_gallery ) {
		$crumbs [] = sprintf( '<a href="%s">%s</a>', psmt_get_gallery_base_url( $component, $component_id ), $my_or_his_gallery );
	}
 ...
 * @return boolean
 */
function psmt_show_gallery_description( $gallery = null ) {

	$gallery = psmt_get_gallery( $gallery );

	// under theme tab in admin panel.
	$show = psmt_get_option( 'show_gallery_description' );

	return apply_filters( 'psmt_show_gallery_description', $show, $gallery );
}
