...
/**
 * PsourceMediathek Gallery class.
 *
 * @since 1.0.0
 *
...
class PSMT_Gallery {
	...
	/**
	 * Maps a DB Object to PSMT_Gallery
	 *
	...
	public function __get( $key ) {
		...
		$value = psmt_get_gallery_meta( $this->id, '_psmt_' . $key, true );
	...
	private function set( $key, $value ) {
		$this->data[ $key ] = $value;
		// update cache.
		psmt_add_gallery_to_cache( $this );
	}
 ...
 *
 * @return PSMT_Gallery|array|null PSMT_Gallery on success or null on failure
 */
function psmt_get_gallery( $gallery = null, $output = OBJECT ) {

	$_gallery      = null;
	$needs_caching = false;

	// if gallery is not given, but we do have current_gallery setup.
	if ( empty( $gallery ) && psourcemediathek()->current_gallery ) {
		$gallery = psourcemediathek()->current_gallery;
	}
	...
	if ( is_a( $gallery, 'PSMT_Gallery' ) ) {
		$_gallery = $gallery;
	} elseif ( is_numeric( $gallery ) ) {
		$_gallery = psmt_get_gallery_from_cache( $gallery );

		if ( ! $_gallery ) {
			$_gallery      = new PSMT_Gallery( $gallery );
			$needs_caching = true;
		}
	} elseif ( is_object( $gallery ) ) {

		// first check if we already have it cached.
		$_gallery = psmt_get_gallery_from_cache( $gallery->ID );
	...
	if ( $needs_caching && ! empty( $_gallery ) && $_gallery->id ) {
		psmt_add_gallery_to_cache( $_gallery );
	}
...
// mind it, psmt is not global group.
/**
 ...
 * @return bool|mixed
 */
function psmt_get_gallery_from_cache( $gallery_id ) {
	return wp_cache_get( 'psmt_gallery_' . $gallery_id, 'psmt' );
}

/**
 * Save gallery object to cache.
 *
 * @param PSMT_Gallery $gallery the gallery object to save to cache.
 */
function psmt_add_gallery_to_cache( $gallery ) {
	wp_cache_set( 'psmt_gallery_' . $gallery->id, $gallery, 'psmt' );
}
 ...
 * @param int $gallery_id numeric gallery id.
 */
function psmt_delete_gallery_cache( $gallery_id ) {
	...
	if ( psmt_get_gallery_from_cache( $gallery_id ) ) {
		wp_cache_delete( 'psmt_gallery_' . $gallery_id, 'psmt' );
	}
}
