...
/**
 * PsourceMediathek Core Component
 * Sets up Galleries/Media for the current page
 */
class PSMT_Core_Component {
	/**
	 * Singleton instance
	 *
	 * @var $this
	...
	/**
	 * Current PsourceMediathek action 'create/upload/manage/
	 *
	 * @var string
	 */
	...
	/**
	 * PsourceMediathek action variables
	 *
	 * @var array
	 */
	...
	private function setup() {
		...
		add_action( 'psmt_setup', array( $this, 'add_rewrite_endpoints' ) );
		// setup galleries.
		add_action( 'psmt_actions', array( $this, 'setup_globals' ), 0 );

		// add context menu to user & groups sub nav.
		add_action( 'bp_member_plugin_options_nav', array( $this, 'context_menu_edit' ) );
		add_action( 'psmt_group_nav', array( $this, 'context_menu_edit' ) );

		add_filter( 'redirect_canonical', array( $this, 'redirect_canonical' ), 10, 2 );
		add_filter( 'psmt_template_redirect', array( $this, 'redirect_attachment' ) );
	...
	public function setup_globals( $args = array() ) {
		...
		$this->component    = psmt_get_current_component();
		$this->component_id = psmt_get_current_component_id();
		...
		psourcemediathek()->the_gallery_query = new PSMT_Gallery_Query();
		psourcemediathek()->the_media_query   = new PSMT_Media_Query();

		if ( ! psmt_is_enabled( $this->component, $this->component_id ) ) {
			return; // do not setup.
		}
		// set the status types allowed for current user.
		$this->accessible_statuses = psmt_get_accessible_statuses( $this->component, $this->component_id, get_current_user_id() );

		$this->status = $this->accessible_statuses;

		// is this sitewide gallery?
		if ( psmt_is_enabled( 'sitewide', $this->component_id ) ) {
		...
		} elseif ( psmt_is_component_gallery() ) {
			// are we on component gallery like groups or events etc?
			$this->action_variables = buddypress()->action_variables;

			$this->setup_component_gallery();
		}
		// fire this action to allow plugins do their own thing on psourcemediathek()->the_gallery_query.
		do_action( 'psmt_setup_gallery_query', $this );

		// once we are here, the basic action variables for psourcemediathek are setup and so
	...
	public function setup_sitewide_gallery() {
		...
		if ( psmt_is_sitewide_gallery_component() ) {
			...
			// setup gallery query.
			psourcemediathek()->the_gallery_query = PSMT_Gallery_Query::build_from_wp_query( $wp_query );
			psourcemediathek()->current_gallery   = psmt_get_gallery( $gallery_id );
		...
		} elseif ( is_post_type_archive( psmt_get_gallery_post_type() ) ) {
	...
	public function setup_user_gallery() {
		...
		if ( psmt_is_enabled( 'members', bp_displayed_user_id() ) && function_exists( 'bp_is_user' ) && bp_is_user() ) {
		...
		} elseif ( psmt_has_gallery_directory() ) {
	...
	public function init() {
		...
		if ( 'create' === $current_action || 'upload' === $current_action ) {
			psourcemediathek()->set_action( $current_action );
			psourcemediathek()->set_edit_action( $current_action );
		...
		if ( 'type' === $current_action && $type_or_status && psmt_is_active_type( $type_or_status ) ) {
			$this->type                   = $type_or_status;
			psourcemediathek()->is_gallery_home = true;
		} elseif ( 'status' === $current_action && $type_or_status && psmt_is_active_status( $type_or_status ) ) {
			$this->status                 = $type_or_status;
			psourcemediathek()->is_gallery_home = true;
		} elseif ( $gallery = psmt_gallery_exists( $current_action, $this->component, $this->component_id ) ) {
			...
			psourcemediathek()->current_gallery = psmt_get_gallery( $gallery );
	...
	public function setup_galleries() {
		...
		$args = apply_filters( 'psmt_main_gallery_query_args', $args );

		// filter out the empty things.
		$args = array_filter( $args );

		psourcemediathek()->the_gallery_query = new PSMT_Gallery_Query( $args );
	...
	public function context_menu_edit() {
		...
		if ( psmt_is_single_media() && psmt_user_can_edit_media( psmt_get_current_media_id(), get_current_user_id() ) ) {

			$url = psmt_get_media_edit_url();
			$links .= sprintf( '<li><a href="%1$s" title ="%2$s"> %3$s</a></li>', $url, _x( 'Medien bearbeiten', 'Profile context menu rel', 'psourcemediathek' ), _x( 'Bearbeiten', 'Profile context menu media edit label', 'psourcemediathek' ) );

		} else {

			if ( psmt_user_can_edit_gallery( psmt_get_current_gallery_id() ) ) {
				$url = psmt_get_gallery_edit_media_url( psmt_get_current_gallery() ); // bulk edit media url.
				$links .= sprintf( '<li><a href="%1$s" title ="%2$s"> %3$s</a></li>', $url, _x( 'Galerie bearbeiten', 'Profile context menu rel attribute', 'psourcemediathek' ), _x( 'Bearbeiten', 'Profile contextual edit gallery menu label', 'psourcemediathek' ) );
			}

			$gallery = psmt_get_current_gallery();

			if ( psmt_user_can_upload( $gallery->component, $gallery->component_id, $gallery ) ) {
				$links .= sprintf( '<li><a href="%1$s" title ="%2$s"> %3$s</a></li>', psmt_get_gallery_add_media_url( psmt_get_current_gallery() ), _x( 'Medien hinzufügen', 'Profile context menu rel attribute', 'psourcemediathek' ), _x( 'Medien hinzufügen', 'Profile contextual add media  menu label', 'psourcemediathek' ) );
			}
		}
	 ...
	 *
	 * @param PSMT_Media $media PsourceMediathek media object.
	 */
	public function setup_single_media_query( $media ) {
		...
		if ( ! is_null( $this->single_media_query ) ) {
			...
			$mp->the_media_query = new PSMT_Media_Query(
				array(
					'id' => $media->ID,
				)
			);
		...
		$mp->current_media = psmt_get_media( $media );
	...
	public function setup_gallery_media_query() {
		...
		$args = apply_filters( 'psmt_main_media_query_args', $args );
		// remove empty.
		$args = array_filter( $args );

		// we are on User gallery home page
		// we do need to check for the access level here and pass it to the query.
		psourcemediathek()->the_media_query = new PSMT_Media_Query( $args );
	}
	...
	public function get_media_id( $slug, $component, $component_id ) {
		...
		$post = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->posts} WHERE post_name = %s and post_type = %s ", $slug, psmt_get_media_post_type() ) );
	...
	public function redirect_canonical( $redirect_url, $requested_url ) {
		...
		if ( is_singular( psmt_get_gallery_post_type() ) && get_query_var( 'paged' ) ) {
	...
	public function redirect_attachment() {
		...
		if ( is_attachment() && psmt_is_valid_media( get_queried_object_id() ) ) {
			$redirect_url = psmt_get_media_url( get_queried_object() );
			psmt_redirect( $redirect_url, 301 );
		}
...
PSMT_Core_Component::get_instance();
