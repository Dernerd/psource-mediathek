 ...
 */
class PSMT_Admin_Settings_Page {
	 ...
	 * @param  string $title Abschnittsüberschrift.
	 * @param  string $desc Section description.
	 *
	 * @return PSMT_Admin_Settings_Panel
	 */
	public function add_panel( $id, $title, $desc = '' ) {

		$panel_id = $id;

		$this->panels[ $panel_id ] = new PSMT_Admin_Settings_Panel( $id, $title, $desc );
	 ...
	 *
	 * @param PSMT_Admin_Settings_Field $field field object.
	 *
	 * @return string
	 */
	...
	public function show_form() {
        ...
        <div class="metabox-holder">
            <div class="postbox options-postbox" style="padding:10px;">
                <form method="post" action="options.php">
					...
					<?php foreach ( $this->panels as $panel ) : ?>
                        ...
                        <div id="<?php echo $panel->get_id(); ?>"
                             class="psmt-settings-panel-tab psmt-admin-options-clearfix">
							...
							<?php foreach ( $sections as $section ) : ?>
                                ...
                                <div id="<?php echo $section_id; ?>"
                                     class="psmt-settings-section-block <?php echo $section_id; ?>">

									<?php do_action( 'psmt_admin_settings_form_top_' . $section_id, $section ); ?>

									<?php $this->do_settings_sections( $this->get_page(), $section_id ); ?>
									<?php do_action( 'psmt_admin_settings_form_bottom_' . $section_id, $section ); ?>
                            ...
                            <div style="padding-left: 10px">
                                ...
                                <input type="button" value="<?php _e( 'Zurücksetzen', 'psourcemediathek' ); ?>"
                                       class="button button-danger psmt-admin-button-danger"

                                       data-warning-message="<?php _e( 'Bist du sicher? Alle PsourceMediathek-Einstellungen werden auf die Standardeinstellungen zurückgesetzt.', 'psourcemediathek' ); ?>"/>
                            </div>
                        </div>
					<?php endforeach; ?>
                ...
                <!-- settings reset form -->
                <form method="post" action="" id="psmt-admin-settings-reset-form">
                    <input type="hidden" name="psmt-action-reset-settings" value="1">
					<?php wp_nonce_field( 'psmt-action-reset-settings' ); ?>
                </form>

                <style type="text/css">
                    .psmt-settings-section-block {
                        padding: 15px 12px;
                        background: #fff;
                    }

                    .psmt-settings-section-block:nth-child(odd) {
                        background: #f8f8f8;
                    }

                    .psmt-settings-section-block h3 {
                        padding: 0 0;
                        font-size: 20px;
                    }

                    .psmt-settings-media-size-field-label {
                        padding: 0 5px;
                    }

                    .psmt-settings-media-size-field {
                        margin-right: 15px;
                        max-width: 65px;
                    }

                    .postbox .psmt-admin-button-danger {
                        background: #DC3D39;
                        color: #fff;
                        border: 0 none;
                        float: right;
                    ...
                    .postbox .psmt-admin-button-danger:hover {
                        background: #FF4742;
                        color: #fff;
                    }

                    .psmt-admin-options-clearfix::after {
                        content: "";
                        display: table;
                        clear: both;
                    }
        ...
        <script>
            jQuery(document).ready(function ($) {
                ...
                if (activetab != '' && $(activetab).length) {
                    ...
                    $('.psmt-settings-panel-tab:first').fadeIn();
                }
                ...
                $('.psmt-admin-button-danger').on('click', function () {
                    var warning_message = $(this).data('warning-message');

                    if (confirm(warning_message)) {
                        $('#psmt-admin-settings-reset-form').submit();
                    }
                    return false;
                })
            });
