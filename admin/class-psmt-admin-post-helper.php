...
class PSMT_Admin_Post_Helper {
	...
	private function setup() {
		...
		//add_filter( 'psmt_upload_default_settings', array( $this, 'plupload_settings' ) );
		add_filter( 'psmt_localizable_data', array( $this, 'add_settings_js' ) );
	}
	...
	public function add_menu() {
		...
		$parent_slug = psmt_admin()->get_menu_slug();

		if( empty( $submenu[ $parent_slug ] ) ) {
			return ;
		}
		//we need to hide the default add new gallery url
		$add_new_url = 'post-new.php?post_type='. psmt_get_gallery_post_type();
		...
		$sub_menu_slug = 'post-new.php?post_type=' . $this->post_type . '&psmt-gallery-type=';

		foreach ( $active_types as $type => $type_object ) {

			$menu_title= sprintf( __( '%s Galerie hinzufügen', 'psourcemediathek' ), psmt_get_type_singular_name( $type )  );
	...
	public function do_metabox( $post ) {
		...
		do_meta_boxes( null, 'psourcemediathek', $post );
	}
	...
	public function show_status_drodown() {
		...
		$selected = $is_new ? psmt_get_default_status() : $gallery->status;

		if ( ! $component && psmt_is_active_component( 'sitewide' ) ) {
			$component = 'sitewide';
		}
		...
		<div class='misc-status-section'>
			<label for="psmt-gallery-status">
				<strong><?php _e( 'Status', 'psourcemediathek' );?></strong>
				<?php psmt_status_dd( array( 'selected' => $selected, 'component' => $component ) );//
				?>
			</label>
			<input type='hidden' name="psmt-gallery-component-id" value="<?php echo $this->get_component_id( $post->ID );?>" />
			<input type='hidden' name="psmt-gallery-type" value="<?php echo $type;?>" />
			<input type='hidden' name="psmt-gallery-component" value="<?php echo $component;?>" />
		</div>
	...
	public function add_metaboxes() {
		...
		add_meta_box(
			...
			_x( 'Galerie Admin', 'Upload Media Box Title', 'psourcemediathek' ), // Title
			array( $this, 'generate_meta_box' ), // Callback function
			$this->post_type, // Admin
			'psourcemediathek', // Context, Our custom context
			'high' // Priority
		);

		//sidebar shortcode show
		add_meta_box(
			'psmt-gallery-shortcode-viewer', // Unique ID
			_x( 'Shortcode', 'Shortcode metabox Title', 'psourcemediathek' ), // Title
	...
	public function generate_shortcode_meta_box( $post ) {
		?>
		<div id="psmt-admin-gallery-shortcode-info">
			<span class="psmt-admin-shortcode-title"><?php echo "[psmt-show-gallery id=" . $post->ID . "]";?></span>
			<p><?php printf( _x( 'Weitere Informationen zu den Shortcode-Optionen findest Du in den <a href="%s">Dokumentationen</a>.', 'admin shortcode message', 'psourcemediathek'), 'https://n3rds.work/docs/psourcemediathek-gallerie-anzeige-shortcodes/' );?>
		</div>
		<?php
	}
	public function generate_meta_box( $post ) {
		...
		psourcemediathek()->the_media_query = new PSMT_Media_Query( array( 'gallery_id' => $post_id, 'per_page' => -1, 'nopaging' => true ) );

		$helper = psmt_admin_edit_gallery_panel_helper();

		$helper->add_panel( array(
			'id'		=> 'add-media',
			'title'		=> __( 'Medien hinzufügen', 'psourcemediathek' ),
			'callback'	=> array( $this, 'generate_upload_meta_box' )
		) );

		$helper->add_panel( array(
			'id'		=> 'edit-details',
			'title'		=> __( 'Details bearbeiten', 'psourcemediathek' ),
			'callback'	=> array( $this, 'edit_details_form' )
		) );
		$helper->add_panel( array(
			'id'		=> 'edit-media',
			'title'		=> __( 'Medien bearbeiten', 'psourcemediathek' ),
	...
	public function load_js () {
		...
		wp_enqueue_script( 'psmt-upload-js', psourcemediathek()->get_url() . 'admin/assets/js/psmt-admin.js', array( 'jquery', 'psmt_uploader' ) );
	}
	...
	public function load_css() {
		...
		$url = psourcemediathek()->get_url();

		wp_register_style( 'psmt-core-css',  $url . 'assets/css/psmt-core.css' );


		wp_register_style( 'psmt-extra-css', $url . 'assets/css/psmt-pure/psmt-pure.css' );
		wp_register_style( 'psmt-admin-css', $url . 'admin/assets/css/psmt-admin.css' );

		wp_enqueue_style( 'psmt-core-css' );
		wp_enqueue_style( 'psmt-extra-css' );
		wp_enqueue_style( 'psmt-admin-css' );
	...
	public function update_gallery_details( $post_id, $post, $update ) {
		...
		if( empty( $gallery->type ) && $type ) {
			wp_set_object_terms( $post_id, psmt_underscore_it( $type ), psmt_get_type_taxname() );
			$gallery->type = $type;
		}
		/**
		...
		$type = empty( $_POST['psmt-gallery-type'] ) ? '' : trim( $_POST['psmt-gallery-type'] ) ;
		$status = empty( $_POST['psmt-gallery-status'] ) ? '' : trim( $_POST['psmt-gallery-status'] ) ;
		$component = empty( $_POST['psmt-gallery-component'] ) ? '' : trim( $_POST['psmt-gallery-component'] ) ;

		if( $type && psmt_is_registered_type( $type ) ) {

			wp_set_object_terms( $post_id, psmt_underscore_it( $type ), psmt_get_type_taxname() );
		}

		if(  $component && psmt_is_registered_component( $component ) ) {

			wp_set_object_terms( $post_id, psmt_underscore_it( $component ), psmt_get_component_taxname() );

		}

		if( $status && psmt_is_registered_status( $status ) ) {

			wp_set_object_terms( $post_id, psmt_underscore_it( $status ), psmt_get_status_taxname() );

		}

		//update media cout or recount?
		if( ! empty( $_POST['psmt-gallery-component-id'] ) ) {

			psmt_update_gallery_meta( $post_id, '_psmt_component_id', absint( $_POST['psmt-gallery-component-id'] ) );
		...
		if( ! $update ) {
			...
			do_action( 'psmt_gallery_created', $post_id );
		}
	...
	private function is_add_gallery() {
		...
		if( isset( $_GET['post_type'] ) && $_GET['post_type'] == $this->post_type && isset( $_GET['psmt-gallery-type'] ) ) {
			return true;
		}
...
//instantiate
PSMT_Admin_Post_Helper::get_instance();
